...
The standalone `laneway_company` wheel is not the `Laneway [Band or Artist]` factory output and is not the `indie_wheel` model. Preserve its light-blue winner-purchase heartbeat, compact accessible centre-button spinning spiral, one-time visibility-aware pale-blue quiz invitation 10 seconds after the first completed result, 10-question Laneway artist quiz and accepted invitation-reveal analytics event only on `dc_b9e7b66620`. Never propagate those presentation rules to artist-specific Laneway or Cool Death editions.
