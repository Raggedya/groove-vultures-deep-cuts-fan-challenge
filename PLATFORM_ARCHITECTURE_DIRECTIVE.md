...
- Owner override dated 2026-08-04: `mahogany-jookbox-master/1` is the only permitted appearance for every new or regenerated Band JookBox. It uses the SHA-256-locked 1120 × 2000 `assets/aggits-jukebox-master-v1.jpg` cabinet with its arched band title, amber ticker, real coin mechanism, privacy-enhanced YouTube screen, four circular icon-only keys, brass `SHARE` plate and lower `Copyright Clearlight Creative 2026.` hardware. `atlas-reference-cabinet/1` is retired and blocked at every Band factory boundary. Already completed non-trial ATLAS editions may remain frozen solely for backward compatibility; they cannot be selected, regenerated, cloned or used as a template. The ten-edition 2026-08-04 trial is the only authorised migration until the owner approves the remaining bands.
