...
- Each edition stores one project identity, one ticker message, one local MP4 and four ordered action slots. The physical top marquee is permanently branded `AGGITS` using the owner-supplied, SHA-256-locked `aggits-marquee-reference-v1.jpg`; its photographed letterforms, arch, spacing, honeycomb brass texture, colouring and ornaments are the immutable source pixels and may never be recreated with a substitute font, SVG or interpreted styling. The project identity is retained for Studio, Library, URLs, QR posters, email and accessibility but never replaces that visible heading. Each action stores enabled state, approved icon ID, display label, action type, raw administrator value, normalized destination and external-tab preference. Public action keys render only the approved icon at a slightly reduced scale; stored labels remain available for Studio and accessible names but never appear beneath the icon. Missing or disabled actions remain inert.
- One accessible coin control presents as an opaque, polished round gold coin, plays the established local real coin-slot recording through the shared dual-path audio engine after direct interaction, requests local video playback only after the recording's `ended` event and prevents overlapping activation. After approximately 2.6 seconds, enabled action keys become usable together in one restrained static state. Aggits Jukebox action keys never pulse, sequence, glow or cast lighting outside their physical faces.
