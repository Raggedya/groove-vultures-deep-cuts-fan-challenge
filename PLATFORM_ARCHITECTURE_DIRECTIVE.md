...
- The owner-supplied icon sheet remains the immutable source for all 110 selectable action icons. Owner approval dated 2026-08-06 supersedes only the standalone Aggits cabinet surface with the versioned SHA-256-locked `aggits-jukebox-oval-master-v2.jpg`; the previous cabinet asset remains preserved for historical compatibility and cannot be selected by the current Windows application.
- Each edition stores one project identity, one ticker message, one selected video and four ordered action slots. The physical top marquee is permanently branded `AGGITS` through the exact pixels in the approved cabinet master; its photographed letterforms, arch, spacing, honeycomb brass texture, colouring and ornaments may never be recreated with a substitute font, SVG or interpreted styling. The project identity is retained for Studio, Library, URLs, QR posters, email and accessibility but never replaces that visible heading. Each action stores enabled state, approved icon ID, display label, action type, raw administrator value, normalized destination and external-tab preference. Public action keys use one row of four black-and-aged-brass oval mechanical keys and render the selected brass icon with its stored visible label. Missing or disabled actions remain inert.
...
- The Windows icon chooser previews those 110 options as the same black-and-aged-brass oval physical keys used by the public cabinet. The action bank has a one-of-four mechanical latch, public-domain clunk and 500-millisecond delayed navigation; no glow, pulse or lighting sequence may be added.
