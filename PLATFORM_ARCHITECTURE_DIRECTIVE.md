...
- The cabinet has the explicit states `sleeping`, `acceptingCoin`, `poweringUp` and `awake`. One accessible coin control plays a local coin-mechanism asset after direct interaction, runs the restrained neon and CRT start-up sequence and stores only the awake flag in edition-specific `sessionStorage`. Refresh restores the awake state without replaying the sound or start-up.
...
- The embedded featured video supplies its own real playback controls and does not autoplay after coin insertion. The user deliberately presses YouTube play after the CRT screen becomes active.
- The biography ticker is configuration-driven, amber, clipped inside its cabinet display and moves left-to-right only after start-up. The eight keys illuminate two at a time by row while awake. Both animations pause in hidden tabs; reduced-motion users receive a simplified screen fade, static biography and gentle non-flashing controls.
