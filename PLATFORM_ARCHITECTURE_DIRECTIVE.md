...
- One accessible coin control presents as a vertical, polished metallic-gold coin and preserves the established local real coin-slot recording byte-for-byte through the shared dual-path audio engine after direct interaction. The existing insertion path, video handoff and overlapping-activation protection remain intact. After approximately 2.6 seconds, enabled action keys become usable together without lighting effects. The keys are mechanically interlocked: pressing one visibly latches it inward while the previously selected key returns outward, plays the local public-domain mechanical clunk, then waits exactly 500 milliseconds before navigating. External destinations continue opening in a secure new tab so the Jukebox remains in its original tab.
...
- The owner manually supplies one title, one ticker, exactly one video source (YouTube URL or local MP4), and exactly four ordered action keys. Every key uses one of the 111 approved brass icons, retains an accessible name and accepts only a validated HTTPS, telephone or email destination.
