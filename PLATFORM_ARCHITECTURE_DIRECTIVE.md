...
- The reusable model name and version are exactly `JookBox` and `jookbox/3`. Filthy Animals `dc_a3c049e4bc` is the owner-locked reference composition and its supplied 762 × 1280 cabinet artwork is protected by SHA-256.
- JookBox uses that full-screen photoreal old-school cabinet as the organising interface. The band identity, broad upper amber biography ticker, privacy-enhanced 16:9 video, centred post-video coin mechanism, red Share hardware, brass Learn More dial and four rows of two selection keys remain aligned to the reference proportions on every viewport.
- The cabinet has the explicit states `sleeping`, `acceptingCoin`, `poweringUp` and `awake`. One accessible coin control plays a locally stored, sourced and licensed real coin-slot recording after direct interaction, runs the restrained neon and CRT start-up sequence and stores only the awake flag in edition-specific `sessionStorage`. Refresh restores the awake state without replaying the sound or start-up.
- The artist-controlled Linktree remains a dated, verified build-time research snapshot. Public keys are selected through an explicit ordered `displaySelectionIds` list containing at most eight verified snapshot entries; the Linktree is never scraped at runtime and unverified entries cannot render. Filthy Animals permanently uses the eight owner-approved ticket, newsletter and YouTube keys shown in its locked cabinet reference.
- `six-key/1` is a separately versioned JookBox key-bank contract. It always renders six uniform physical positions in a three-by-two grid and requires four to six verified external destinations. External destinations retain their configured order. If fewer than six are available, `Learn More` fills the first gap and `Share` fills the next; production fails closed below four verified external destinations. The six keys illuminate gently one at a time in reading order: top-left, top-centre, top-right, bottom-left, bottom-centre, bottom-right, then repeat. Replacement keys inherit the edition's existing key appearance exactly and must never trigger a restyle of an approved edition.
...
- The biography ticker is configuration-driven, large, bright amber, clipped inside the broad upper cabinet display and moves right-to-left in natural reading order only after start-up. Before the coin interaction, all eight wider selection keys remain visibly dimmed, unfocusable and unavailable; after activation they illuminate brightly one at a time in sequence and become usable only when the cabinet reaches `awake`. Both animations pause in hidden tabs; reduced-motion users receive a simplified screen fade, static biography and gentle non-flashing controls.
- Share and the sourced second-screen biography remain available through the red button and brass dial on classic JookBoxes. In `six-key/1`, a utility moved into an empty key position suppresses only its duplicate standalone control.
...
- ATLAS `dc_e22f1cb651` alone uses the owner-approved `atlas-reference-cabinet/1`: a locked 887 × 1774 photoreal black-walnut and chrome cabinet maps the live marquee, biography ticker, left coin mechanism, YouTube screen, verified destination keys, Learn More control, prominent `Support Our Band` share action and status display into dedicated physical recesses. The share action reads `Please share our JookBox`, includes the heart plus a small share icon, and replaces the former standalone lower Share hardware; that lower cabinet position reads `Copyright Clearlight Creative 2026.` without replacing or changing the immutable global footer. Its exact existing key shape, typography, spacing, colours, borders, dimensions and resting/active appearance are a permanent visual invariant. ATLAS uses `six-key/1`: its five verified external keys are followed by Learn More, and the existing gentle light advances one key at a time in reading order. Missing website and merchandise destinations remain omitted. This exception must never alter the locked Filthy Animals composition or become the factory default.
