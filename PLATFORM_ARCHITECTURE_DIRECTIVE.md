...
- `ADD 10 BANDS` remains a private, unpublished lead-intake boundary. A direct, resolved Bandcamp artist page discovered for the named band is sufficient to create a 75%-confidence manual-review draft; Instagram, Facebook, Spotify and a sourced biography are optional enrichment at intake and must remain visibly blank when they cannot be verified. Intake must neither search for nor require YouTube. Keys remain Bandcamp, Instagram, Facebook and Spotify in that exact order, the discovered artist name populates the heading, and the draft is created with local MP4 selected as its empty primary media input. The owner must review identity and complete or correct missing data before publication. Production remains fail-closed: all four valid destinations, ticker and media plus the normal 98% publication validation must pass before the edition can go live. Never invent or silently substitute a destination.
