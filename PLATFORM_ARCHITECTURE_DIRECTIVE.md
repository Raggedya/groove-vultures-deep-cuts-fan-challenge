...
- JookBox uses a full-screen photoreal old-school illuminated cabinet with subtly weathered polished wood, aged chrome/gold trim, restrained period lighting and a privacy-enhanced 16:9 featured-video hero. The dynamic band name is an auto-fitting, single-line illuminated cabinet sign; the small `JookBox` model mark remains subordinate. The coin and visible slot are mounted above the video, and Share is a cabinet control beside the one-line video metadata rather than a generic bottom-bar action. Duplicate band plaques, visible helper copy and sound-toggle controls are absent.
...
- The artist-controlled Linktree is retained as a dated, verified build-time research snapshot, but it never controls the public button set. JookBox renders only these verified categories, in this order: `Bandcamp`, `Spotify`, `Instagram`, `Band Website`, `Buy Music`, `Buy Merch`, `Facebook`. Missing categories are omitted and the remaining tactile controls rebalance automatically. `Buy Music` and `Buy Merch` require separately verified direct store destinations.
- The embedded featured video supplies its own real playback controls; no separate YouTube, show, newsletter, TikTok, contact, podcast, reaction-video or other Linktree button may render. One dominant primary action promotes the first available approved category. Every action retains evidence, and runtime scraping never publishes unverified destinations.
- `Learn More` is a normal cabinet selection beneath the verified destination deck, never a sticky bottom bar. It opens a second in-page screen containing only sourced band biography copy and a verified official source.
