...
- The drag/click/keyboard coin interaction, sourced real coin sound, four-state session model, neon/CRT start-up, title fitting and reduced-motion behavior reuse the shared JookBox engine. Bar Edition alone uses its locked three-stage paired incandescent sequence across the six physical keys; Band JookBox retains its independent single-key sequence. The yellow coin may pulse only while the Bar Edition cabinet is sleeping; once activation begins it inserts and disappears, and it never pulses or glows in an awake or session-restored cabinet. The MP4 playback request occurs synchronously inside the coin gesture and retains native controls when browser autoplay is blocked.
