...
- **Secure Publish Venue** is the only Venue Library path to a live Bar Edition. One owner-controlled Windows installation is activated by an emailed six-digit code; its device-scoped publisher credential is encrypted by the operating system and never stored in the venue library or repository. Publish Venue validates exactly five HTTPS destinations plus the internal About Us key, ticker, About Us copy and MP4; uploads versioned assets through the private Cloudflare publisher; registers only that isolated edition; scan-tests the permanent QR; confirms delivery email; and verifies the live page, MP4 and QR before recording `published`. The owner performs no GitHub or Cloudflare step. A failed or interrupted job stays unpublished and restores the previous live edition record.
