...
- Final owner approval dated 2026-08-04: the product name is **Mahogany Jukebox Master** and `mahogany-jookbox-master/1` is the only permitted appearance for every new, migrated or regenerated Band JookBox. Its approved live reference is Savage Garden `dc_e65763b78b` after the final mobile coin-motion and four-key proportion corrections. It uses the SHA-256-locked `assets/aggits-jukebox-master-v1.jpg` cabinet with its arched band title, amber ticker, real coin mechanism, privacy-enhanced YouTube screen, four circular icon-only keys, brass `SHARE` plate and lower `Copyright Clearlight Creative 2026.` hardware. `atlas-reference-cabinet/1`, neon and every other Band appearance are retired and blocked at every current Band factory boundary. Previously completed alternatives remain frozen solely for backward compatibility; they cannot be selected, regenerated, cloned or used as a template. The ten-edition trial is complete, and all future Band work uses this master.
- The cabinet has the explicit states `sleeping`, `acceptingCoin`, `poweringUp` and `awake`. One accessible coin control plays the locally stored, sourced and licensed real coin-slot recording at full audible level after direct interaction, runs the restrained neon and CRT start-up sequence and stores only the awake flag in edition-specific `sessionStorage`. Every Band, Bar, Aggits and Studio renderer must use the shared coin-audio engine: HTML media begins synchronously inside the coin gesture for mobile reliability, pre-decoded Web Audio remains the fallback, and the completion promise resolves only on the recording's real `ended` event. Video media may not request focus until that recording completes. Electronic synthesis, silent failure handlers and renderer-specific substitute sounds are forbidden. Refresh restores the awake state without replaying the sound or start-up.
- The artist-controlled source remains a dated, verified build-time research snapshot. New Mahogany Jukebox Master editions expose exactly four verified snapshot entries through an explicit ordered `displaySelectionIds` list; sources are never scraped at runtime and unverified entries cannot render. Filthy Animals permanently retains its eight owner-approved legacy ticket, newsletter and YouTube keys.
- `mahogany-four-key/1` is the permanent new-edition Band JookBox key-bank contract. It renders exactly four uniform circular, icon-only physical keys in one row and requires exactly four independently verified external destinations. Stored labels remain available to accessibility and analytics but never render beneath the icons. Band keys do not pulse, sequence, glow or illuminate; Bar and other isolated product contracts are unaffected.
- The live band marquee measures its rendered text after fonts load and whenever its cabinet resizes. Names that fit retain the approved mahogany size; longer names are progressively reduced and, only when necessary, balanced across multiple lines inside the same top display. A future band name must never overflow, clip or replace the locked cabinet composition.
- The embedded featured video supplies its own real playback controls and may request playback only after the real coin recording's `ended` event, preventing mobile media-focus interruption. Browser autoplay policy may still block audible playback, so the standard YouTube play control always remains visible as the reliable manual fallback; restored sessions do not replay or request autoplay.
...
- The biography ticker is configuration-driven, large, bright amber, clipped inside the broad upper cabinet display and moves right-to-left in natural reading order only after start-up. Before the coin interaction, all selection keys remain visibly dimmed, unfocusable and unavailable; when the cabinet reaches `awake`, all four keys unlock together without a glow, pulse or light sequence. Continuous ticker animation pauses in hidden tabs; reduced-motion users receive a simplified screen fade and static biography.
- The permanent mahogany Band cabinet uses its full-width brass plate as the sole working share action, visibly labelled exactly `SHARE`. Its lower status hardware reads `Copyright Clearlight Creative 2026.` without replacing or changing the immutable global footer.
- JookBox never displays Aggits, a quiz or a spinning wheel.
- Owner override dated 2026-08-04: new and trial-migrated Band JookBox delivery packages use the SHA-256-locked 1254 × 1254 character QR poster with perspective-fitted matrix (`aggits-character-poster-perspective/2`). This exception applies only to the emailed/downloadable QR poster; Aggits remains absent from the live Band JookBox page, its discovery artwork and its interaction model. The permanent opaque HTTPS route, fitted band title, four-module quiet zone, quadrilateral cabinet-panel fit and actual full-size, 627-pixel and 360-pixel decoder scan-back are fail-closed publication requirements. Existing completed delivery artwork remains unchanged unless separately republished by the owner.
- Owner lock dated 2026-08-10: all newly generated Band, Aggits and Mahogany VU Jukebox QR posters use `mahogany-cabinet-integrated-perspective/4`. The title is rendered directly into the photographed brass-framed leather plaque without a substitute rectangle, and the high-contrast QR is recessed inside the existing parchment surround and projected module-by-module into its photographed four-corner perspective with its four-module quiet zone intact. Flat white square overlays, edge-to-edge pasted panels and pasted-on title labels are forbidden. Each other Jukebox product must likewise follow the native geometry, material, lighting and depth of its own locked QR master; a square matrix is acceptable only when its photographed host panel is genuinely front-facing and square. Existing opaque URLs remain unchanged, and full-size, 627-pixel and 360-pixel decoder scan-back remains fail-closed.
...
- ATLAS `dc_e22f1cb651` and other pre-override ATLAS Band editions are frozen historical products only. They remain isolated for route compatibility but can never supply the current Band template, be regenerated through the factory or be selected for a new edition.
...
- The owner-supplied icon sheet remains the immutable source for its original 110 selectable action icons. Owner approval dated 2026-08-06 adds one matching antique-brass Bandcamp mark, creating a 111-icon catalog, and supersedes only the standalone Aggits cabinet surface with the versioned SHA-256-locked `aggits-jukebox-oval-master-v2.jpg`; previous cabinet and icon assets remain preserved for historical compatibility and cannot be selected by the current Windows application.
- Each edition stores one project identity, one ticker message, one selected video and four ordered action slots. The physical top marquee is permanently branded `AGGITS` through the exact pixels in the approved cabinet master; its photographed letterforms, arch, spacing, honeycomb brass texture, colouring and ornaments may never be recreated with a substitute font, SVG or interpreted styling. The project identity is retained for Studio, Library, URLs, QR posters, email and accessibility but never replaces that visible heading. Each action stores enabled state, approved icon ID, accessible name, action type, raw administrator value, normalized destination and external-tab preference. Public action keys use one row of four black-and-aged-brass oval mechanical keys and render only the selected centred brass icon; the accessible name remains available to assistive technology and analytics but is never drawn on the physical key. Missing or disabled actions remain inert.
- One accessible coin control uses the SHA-256-locked transparent `aggits-coin-gold-v1.png`: a face-visible, realistic metallic-gold coin with the embossed Aggits portrait and `IN AGGITS WE TRUST`. It sits immediately beside the vertical slot with approximately ten percent of its face already crossing the slot edge, then travels left and turns edge-on into the slot. It preserves the established local real coin-slot recording byte-for-byte through the shared dual-path audio engine after direct interaction. The existing video handoff and overlapping-activation protection remain intact. After approximately 2.6 seconds, enabled action keys become usable together. While media is genuinely playing, local MP4 editions use a Web Audio analyser over 40–180 Hz to apply a restrained, internally clipped warm-amber bass response to the four keys; YouTube and security-restricted media use a capped timing fallback only while confirmed as playing. The response stops and clears on pause, mute, end, page hiding or teardown, and reduced-motion removes scaling while retaining gentle illumination. The keys remain mechanically interlocked: pressing one visibly latches it inward while the previously selected key returns outward, plays the local public-domain mechanical clunk, then waits exactly 500 milliseconds before navigating. External destinations continue opening in a secure new tab so the Jukebox remains in its original tab.
- The current photographed video opening is landscape `3759:2992`; the approved practical Camtasia canvas and H.264 export are `1804 × 1436` pixels (width × height). Media fills that opening edge to edge with cover rendering and no browser borders. Only the media element's genuine `ended` event is treated as completion; start-up timers must never limit playback duration or hide final frames or audio.
...
- The physical top marquee is permanently branded `AGGITS` through the immutable owner-supplied `aggits-marquee-reference-v1.jpg`. Its exact photographed typography, proportions, arch, texture, colour and ornamentation are preserved byte-for-byte; generated text and substitute fonts are forbidden. A project name remains mandatory for library identity, permanent URLs, QR posters, emails and accessibility, but it must never replace or alter the visible `AGGITS` cabinet heading.
- The owner manually supplies one title, one ticker, exactly one video source (local MP4 as the primary/default workflow, or an owner-chosen YouTube URL), and exactly four ordered action keys. Every key uses one of the 111 approved brass icons, retains an accessible name and accepts only a validated HTTPS, telephone or email destination.
- The Windows icon chooser previews those 111 options—including Bandcamp—as the same clean, single-bezel black-and-aged-brass oval physical keys used by the public cabinet. The action bank has a one-of-four mechanical latch, public-domain clunk and 500-millisecond delayed navigation; no glow, pulse, secondary exterior line, visible key label or lighting sequence may be added.
...
- `ADD 20 BANDS` is a private, unpublished candidate-intake boundary. Its primary shortlist comes from one structured Wikidata query requiring Linktree, Bandcamp, Instagram, Facebook and Spotify identifiers on a musical-group record; general web search is only a fallback lead source. Every constructed destination must still resolve directly and reach the existing 98% identity gate before a draft is written. An identity-verified embeddable official YouTube video makes the draft gold and preloads the verified video layer; the same four verified destinations without an eligible video make it silver with empty MP4 media for owner completion. Keys remain Bandcamp, Instagram, Facebook and Spotify in that exact order. Intake never publishes. Library deletion is recoverable archival for projects that have never received a public identity; any published, unpublished or identity-reserved project is permanently protected from deletion.
...
## Fullnoise VU desktop product contract
