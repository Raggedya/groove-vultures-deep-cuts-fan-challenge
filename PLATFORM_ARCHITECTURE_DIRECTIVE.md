...
- JookBox uses that full-screen photoreal old-school cabinet as the organising interface. The band identity, top coin mechanism, privacy-enhanced 16:9 video, red Share hardware, amber biography ticker, brass Learn More dial and four rows of two selection keys remain aligned to the reference proportions on every viewport.
...
- The embedded featured video supplies its own real playback controls and requests playback four seconds after direct coin activation. Browser autoplay policy may block delayed audible playback, so the standard YouTube play control always remains visible as the reliable manual fallback; restored sessions do not replay or request autoplay.
- The biography ticker is configuration-driven, bright amber, slightly enlarged, clipped inside its cabinet display and moves right-to-left in natural reading order only after start-up. The eight keys illuminate two at a time by row while awake. Both animations pause in hidden tabs; reduced-motion users receive a simplified screen fade, static biography and gentle non-flashing controls.
- Share and the sourced second-screen biography remain available through the red button and brass dial without adding generic navigation or changing the reference layout.
