...
- [x] Lock the isolated ATLAS reference cabinet: a photoreal 887 × 1774 black-walnut, chrome and neon machine with live marquee, hero biography ticker, left coin bay, large YouTube screen, three-by-two verified key bank, sourced Learn More, a prominent working `Support Our Band` share action with small share icon, and in-cabinet `Copyright Clearlight Creative 2026.` hardware; preserve its exact key styling while advancing the light gently one key at a time without changing Filthy Animals, the global footer or the factory default.
