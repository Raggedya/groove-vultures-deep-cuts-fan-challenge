...
The cabinet starts in `sleeping`, accepts one accessible coin, plays a local coin-mechanism sound after that gesture, moves through `acceptingCoin` and `poweringUp`, then reaches `awake`. Neon hesitates and settles, the screen uses a restrained CRT reveal, selection keys illuminate two at a time by row and the sourced uppercase biography travels left-to-right across the amber ticker. The awake flag is edition-specific session state: a refresh restores the live cabinet without replaying sound or start-up. YouTube remains user-controlled and never autoplays. Hidden tabs pause continuous animation; reduced motion replaces the theatrical sequence with a gentle fade, static biography and non-flashing controls. Missing audio logs a development warning and never blocks the visual sequence.
