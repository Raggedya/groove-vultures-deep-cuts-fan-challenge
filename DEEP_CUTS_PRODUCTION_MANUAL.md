...
JookBox uses `editionType: jukebox` and locked model `jookbox/3` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. Filthy Animals remains an immutable completed legacy edition with its SHA-256-protected 762 × 1280 artwork. The permanent new-edition default is `atlas-reference-cabinet/1`, using the shared SHA-256-locked 887 × 1774 black-walnut, chrome and neon cabinet first approved on ATLAS.
