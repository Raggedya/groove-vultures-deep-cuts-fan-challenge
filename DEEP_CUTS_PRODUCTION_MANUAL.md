...
Bar Edition is the static `bar_jukebox` Studio workflow. It accepts one venue name, exactly five administrator-labelled HTTPS destinations, administrator ticker copy and one local MP4 up to 500 MB. It performs no web lookup, accepts no YouTube link, and always supplies Share as the sixth key. Inside the cabinet, the lower arch marquee shows only the fitted venue name and `JOOKBOX` tagline immediately above the ticker; it never displays `BAR EDITION`. The support strip below the keys is also a working `Share [Venue Name] with your mates` control after coin activation. Its preview uses the locked ATLAS cabinet and the shared drag/click/keyboard coin, real coin sound, neon/CRT, ticker and single-key sequence. The yellow coin pulses only before activation, then inserts and remains absent without pulse or glow while awake, including after an awake-session refresh. A direct coin gesture requests immediate MP4 playback; native controls remain the fallback. Because the private preview is bound to ephemeral `127.0.0.1`, Studio does not create or download a Bar Edition preview QR. It shows a publication-pending panel; production generates and scan-tests the permanent QR only after deployment provides the public HTTPS URL. See `BAR_EDITION_MODEL.md`.
