...
Each JookBox edition starts from one artist-controlled source and captures its visible entries as dated research evidence, then independently verifies every destination that may be published. Runtime scraping never creates controls. New mahogany editions expose exactly four verified snapshot entries through an explicit ordered `displaySelectionIds` list; unavailable or stale entries are omitted. Frozen legacy products retain their completed key layouts.

The permanent `mahogany-four-key/1` format presents exactly four uniform circular icon-only keys in one row. It accepts exactly four verified external destinations in configured order. Visible labels and supporting text are suppressed while accessible names and analytics metadata remain. Band keys use no light sequence, pulse, glow or active halo.

The top band marquee is data-driven and automatically fitted after the display font is ready and whenever the cabinet changes size. A name that fits keeps the approved mahogany scale. A longer name is reduced only as much as necessary and may balance across multiple lines within the existing marquee; it must never be truncated, overflow the frame or change the locked cabinet proportions. Studio previews use the same fitting rule.

The cabinet starts in `sleeping`, accepts one accessible coin centred beneath the video, plays a locally stored, sourced and licensed real coin-slot recording at full audible level after that gesture, moves through `acceptingCoin` and `poweringUp`, then reaches `awake`. Band, Bar, Aggits and Studio previews all use the same coin-audio engine: HTML media starts synchronously inside the visitor's coin gesture, pre-decoded Web Audio remains the fallback, and the shared completion promise settles only after the recording's genuine `ended` event. Video media focus is requested only after that event so iPhone playback cannot cut off the mechanical coin drop. Silent failure handlers, synthetic electronic replacements and per-renderer substitute sounds are forbidden. While sleeping, every selection key remains clearly dimmed, removed from keyboard navigation and unavailable; the coin interaction begins the staged illumination, and links unlock only at `awake`. Neon hesitates and settles, the screen uses a restrained CRT reveal, and the sourced uppercase biography travels right-to-left in natural reading order across the large bright amber hero ticker above the video. The privacy-enhanced YouTube player retains its real play control because a browser may still block audible autoplay. The awake flag is edition-specific session state: a refresh restores the live cabinet without replaying sound, start-up or autoplay. Hidden tabs pause continuous animation; reduced motion replaces the theatrical sequence with a gentle fade and static biography. Missing audio logs a development warning and never blocks the visual sequence.
...
The locked mahogany cabinet uses its full-width brass plate as the sole working Share control and displays exactly `SHARE`. Its lower status hardware reads `Copyright Clearlight Creative 2026.` while the immutable global footer stays unchanged. There is no duplicate share key, generic primary action, sticky action bar, visible helper, duplicate plaque or sound toggle.
...
Aggits Jukebox publication uses its own protected publisher, D1 records and R2 key prefix. The single `PUBLISH + EMAIL QR` action saves the edition, validates the title, ticker, one-to-four enabled actions and public MP4 ceiling, assigns or reuses one stable opaque edition ID, uploads the versioned media, and generates the permanent QR from the deployed HTTPS URL. It then automatically requests the completion email without another owner action. The QR renderer uses the SHA-256-locked 1254 x 1254 owner artwork, fits the edition title into the upper plaque, and places one centred QR insert inside the measured bright opening with balanced margins and no frame overlap. It scan-tests the completed artwork at full and phone-size resolution. The completion email names the edition, provides the permanent URL as both a clickable link and plain copyable text, and attaches the fitted QR image. Delivery confirmation plus live page, video and QR verification are mandatory before Studio reports success; failure rolls back without changing an existing live version. The current standalone output is the locked `aggits-jukebox-oval-master/4` cabinet presentation with a vertical metallic-gold coin and four icon-only black/brass oval keys using the 111-icon clean oval-native vector set, including Bandcamp. Accessible names remain stored but are not drawn on the keys, and the single bezel never gains a secondary exterior line. Exactly one key may remain mechanically depressed; its local public-domain clunk begins immediately and navigation follows 500 milliseconds later. The key bank never glows, pulses or sequences. Create performs a real YouTube IFrame Player availability check and fails closed when the owner blocks embedded playback.

Mahogany Jukebox candidate intake uses the large `ADD 20 BANDS` control with no operator search parameters. One structured Wikidata query shortlists musical groups already carrying Linktree, Bandcamp, Instagram, Facebook and Spotify identifiers; direct page verification then applies the normal 98% identity gate, while general web search remains fallback-only. It writes unpublished drafts only. Gold means all four destinations plus an eligible embeddable official YouTube video; silver means all four destinations with no eligible video. Every other result is rejected. `Delete Jukebox` moves never-published drafts into the local archive; projects with a reserved or published public identity cannot be deleted.
