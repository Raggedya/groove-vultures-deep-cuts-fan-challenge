...
JookBox uses `editionType: jukebox` and model `jookbox/2` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. Its isolated old-school cabinet uses wood, chrome/gold and restrained period illumination, keeps the verified 16:9 featured YouTube video as the visual hero, and renders the artist-controlled Linktree as large physical selection controls.
