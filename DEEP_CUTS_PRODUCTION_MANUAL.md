...
Bar Edition is the static `bar_jukebox` Studio workflow. It accepts one venue name, exactly five administrator-labelled HTTPS destinations, administrator ticker copy and one local MP4 up to 500 MB. It performs no web lookup, accepts no YouTube link, and always supplies Share as the sixth key. Its preview uses the locked ATLAS cabinet and the shared drag/click/keyboard coin, real coin sound, neon/CRT, ticker and single-key sequence. A direct coin gesture requests immediate MP4 playback; native controls remain the fallback. See `BAR_EDITION_MODEL.md`.
