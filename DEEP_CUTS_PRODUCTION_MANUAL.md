...
JookBox uses `editionType: jukebox` and model `jookbox/2` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. Its isolated old-school cabinet uses subtly weathered wood, aged chrome/gold and restrained period illumination, keeps the verified 16:9 featured YouTube video as the visual hero, and renders the artist-controlled Linktree as large physical selection controls. The dynamic band name leads the marquee above the smaller JookBox model mark; the user-facing coin and slot sit above the video, and Share is mounted as cabinet hardware beside the video metadata.

Each JookBox edition starts from one artist-controlled Linktree, captures its current visible buttons as a dated build-time snapshot, then independently verifies every published destination. Search results are never buttons. Verified show destinations are grouped into one expandable Upcoming Shows module; the remaining actions automatically align in the tactile two-column control deck, and missing entries are omitted. The primary action promotes shows, then falls back through Spotify, YouTube, the official website and newsletter. If an official YouTube channel exists, the embedded feature is its most-viewed embeddable official video at verification time. A user-pushed coin unlocks the destinations and initiates autoplay, sound and a short cabinet light-up sequence. Only controls with real behaviour may render. Ambient motion is restrained, respects reduced motion and never interferes with keyboard focus or outbound navigation. `Learn More` uses separately verified official biography evidence.
