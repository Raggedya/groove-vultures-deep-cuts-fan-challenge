...
JookBox uses `editionType: jukebox` and locked model `jookbox/3` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. Filthy Animals remains the sole immutable legacy-cabinet exception, with its SHA-256-protected 762 × 1280 artwork. Every new JookBox must use `atlas-reference-cabinet/1` and `six-key/1`; factory input cannot select or restore the retired cabinet. The mandatory shared presentation is the SHA-256-locked 887 × 1774 black-walnut, chrome and neon cabinet first approved on ATLAS.

The owner-approved delivery exception is `aggits-character-poster/1`: every newly configured Band JookBox receives the locked 1254 x 1254 character QR poster while the live page remains completely Aggits-free. Its band title is fitted into the physical plaque and the permanent opaque URL is rendered as a centred high-contrast QR inside the measured opening. Publication fails unless the final poster decodes at 1254, 627 and 360 pixels.

Each JookBox edition starts from one artist-controlled source and captures its visible entries as dated research evidence, then independently verifies every destination that may be published. Runtime scraping never creates controls. New editions expose four to six verified snapshot entries through an explicit ordered `displaySelectionIds` list; unavailable or stale entries are omitted. Filthy Animals permanently retains its eight owner-approved legacy keys.

The permanent `six-key/1` format always presents six uniform keys in a three-by-two grid. It accepts four to six verified external destinations in configured order. Learn More fills the first unoccupied key and Share fills the next; fewer than four verified external destinations fail validation. The light advances gently through one key at a time in reading order. The ATLAS-approved key shape, typography, alignment, spacing, colours, borders, dimensions and glow treatment are immutable across every new edition.

The top band marquee is data-driven and automatically fitted after the display font is ready and whenever the cabinet changes size. A name that fits keeps the approved ATLAS scale. A longer name is reduced only as much as necessary and may balance across multiple lines within the existing marquee; it must never be truncated, overflow the frame or change the locked cabinet proportions. Studio previews use the same fitting rule.

The cabinet starts in `sleeping`, accepts one accessible coin centred beneath the video, plays a locally stored, sourced and licensed real coin-slot recording at full audible level after that gesture, moves through `acceptingCoin` and `poweringUp`, then reaches `awake`. Band, Bar, Aggits and Studio previews all use the same coin-audio engine: the recording is pre-decoded for Web Audio, its audio context resumes inside the direct gesture, and HTML audio remains the browser fallback. Silent failure handlers, synthetic electronic replacements and per-renderer substitute sounds are forbidden. While sleeping, every selection key remains clearly dimmed, removed from keyboard navigation and unavailable; the coin interaction begins the staged illumination, and links unlock only at `awake`. Neon hesitates and settles, the screen uses a restrained CRT reveal, wider selection keys illuminate brightly one at a time and the sourced uppercase biography travels right-to-left in natural reading order across the large bright amber hero ticker above the video. The privacy-enhanced YouTube player requests playback synchronously inside the direct coin interaction, with no post-coin delay; the real YouTube play control remains visible because a browser may still block audible autoplay. The awake flag is edition-specific session state: a refresh restores the live cabinet without replaying sound, start-up or autoplay. Hidden tabs pause continuous animation; reduced motion replaces the theatrical sequence with a gentle fade, static biography and non-flashing controls. Missing audio logs a development warning and never blocks the visual sequence.

One origin-aware Core Engine helper owns JookBox external-link behaviour. Verified external destinations open in a new tab with `target="_blank"` and `rel="noopener noreferrer"` so the powered cabinet and its mobile scroll position remain available in the original tab; relative and same-origin navigation is not rewritten.

The locked six-key cabinet uses a working `Support Our Band` / `Please share our JookBox` panel with the heart and small share icon. Its lower status hardware reads `Copyright Clearlight Creative 2026.` while the immutable global footer stays unchanged. When Learn More or Share occupies a fallback key, only its duplicate standalone control is hidden. There is no generic primary action, sticky action bar, visible helper, duplicate plaque or sound toggle.
...
ATLAS `dc_e22f1cb651` remains the first completed reference for the permanent shared appearance. Southern Culture on the Skids `dc_f3f4750b1b` is the first factory edition corrected to it. Every edition supplies its own independently verified identity, video, destinations, ticker and biography; no edition depends directly on another edition's configuration. Promoting the shared asset and visual contract never changes Filthy Animals or other completed products.
...
Bar Edition is the static `bar_jukebox` Studio workflow. It accepts one venue name, exactly five administrator-labelled HTTPS destinations, administrator fallback ticker copy, administrator-approved About Us copy and one local MP4 up to 500 MB for local preview; production MP4 assets are fail-closed at 24 MiB. It accepts no YouTube link and always supplies an internal About Us control as the sixth key. Inside the cabinet, the arch marquee shows only the curved, automatically fitted venue name and `JOOKBOX` tagline; it never displays `BAR EDITION`. The support strip below the keys is the sole working Share control after coin activation and its visible label is exactly `SHARE`. Its preview uses the locked heritage brass cabinet and the shared drag/click/keyboard coin, sourced real coin-slot-and-fall recording, neon/CRT and ticker. Its six period SVG keys illuminate individually in reading order at 1.1-second intervals; the warm incandescent lamp is clipped inside the parchment face and never blooms outside the bronze frame. The yellow coin pulses only before activation, then inserts and remains absent without pulse or glow while awake, including after an awake-session refresh. A direct coin gesture requests immediate MP4 playback; native controls remain the fallback. The private Venue Library reads the configured official Gigs destination at publication time, accepts supported structured upcoming-event facts only, and stores the resulting static ticker; when none are available it preserves the approved fallback. No lookup runs in the public page. Because the private preview is bound to ephemeral `127.0.0.1`, Studio does not create or download a Bar Edition preview QR. A complete draft exposes one `SAVE VENUE + PUBLISH` control that upserts its stable library record, integrity-checks the MP4 and queues the same protected publisher; it does not bypass any validation or security gate. The private library exposes one `Published` / `Unpublished` switch. First publication runs the complete fail-closed Cloudflare, R2, QR, email and live-verification pipeline. Unpublishing hides the live route while preserving the opaque edition ID, URL, QR payload, versioned assets and local record; republishing validates and reactivates that same identity. The one-time active-library reset archives and hides existing records without touching their public state, and `Add Venue` restores one preserved venue at a time. See `BAR_EDITION_MODEL.md`.

The locked Bar visual surface is `assets/jookbox-bar-heritage-brass-v1.png`, protected by SHA-256. It presents the existing interaction as a worn mahogany and aged-brass venue jukebox: venue identity in the arch, a large bright yellow single-line ticker, local MP4 in the main screen, exactly six parchment/brass keys, the sole wide share plate and an in-cabinet brass Clearlight copyright plate. This is a Bar-only renderer skin; it does not change the stored backward-compatible appearance identifier and can never affect Band JookBox.
...
Aggits Jukebox publication uses its own protected publisher, D1 records and R2 key prefix. The single `PUBLISH + EMAIL QR` action saves the edition, validates the title, ticker, one-to-four enabled actions and public MP4 ceiling, assigns or reuses one stable opaque edition ID, uploads the versioned media, and generates the permanent QR from the deployed HTTPS URL. It then automatically requests the completion email without another owner action. The QR renderer uses the SHA-256-locked 1254 x 1254 owner artwork, fits the edition title into the upper plaque, and places one centred QR insert inside the measured bright opening with balanced margins and no frame overlap. It scan-tests the completed artwork at full and phone-size resolution. The completion email names the edition, provides the permanent URL as both a clickable link and plain copyable text, and attaches the fitted QR image. Delivery confirmation plus live page, video and QR verification are mandatory before Studio reports success; failure rolls back without changing an existing live version. The public four-button cabinet renders only the approved icons at a reduced scale, enables them in one static non-illuminated state after start-up, and uses an opaque polished-gold coin.

Use Studio's **Import Editions** screen for administrator-supplied Aggits Jukebox master data. Always run preflight first, correct or exclude invalid rows, select at most 1,000 eligible records and explicitly confirm draft creation. Download the reconciliation CSV after the batch. The importer never publishes; open and complete each draft, then use the protected publisher individually. Reimports use the stable source identities and an explicit skip, update-drafts or create-new-only mode. See `docs/AGGITS_JUKEBOX_BULK_IMPORT.md` for the schema, mappings, reports and recoverable rollback procedure.
...
The Laneway company edition additionally records intentional wheel spins, completed artist selections, winner-video impressions, winner-versus-directory Spotify clicks by verified artist, anonymous directory searches, quiz starts, answers, completions, abandonments and replays, and services-contact intent. The Worker accepts only a versioned allow-list of scalar metadata. Duplicate event IDs are ignored by D1, reports reconcile against the raw event audit, every verified roster artist is included even when the result is zero, and report generation fails closed instead of silently truncating data.
...
The Laneway company reference uses one restrained pale-teal/light-blue accent system across decorative headings, centre-ring illumination, winner orbit, purchase controls, artist video, quiz invitation, quiz surfaces and result actions. Red is reserved solely for the illuminated wheel pointer. This palette rule is isolated to `laneway_company`; it does not recolour Cool Death Records, artist-specific Laneway editions or any other completed product.
...
After a Laneway company-wheel result, a verified winner video may appear beneath the impact and purchase controls and above the delayed quiz invitation. The isolated `artist-videos.json` map is keyed by roster name and may contain only playable YouTube videos evidenced by the artist's official Laneway Music profile (or an already owner-approved edition video). When a profile supplies several playable music-video candidates, select the candidate with the highest public view count at verification time. Never claim this is a permanent global YouTube ranking. Remove the iframe source on a new spin, load YouTube only for the selected artist, use `youtube-nocookie.com`, and omit the entire panel when verification is absent or playback is unavailable.
