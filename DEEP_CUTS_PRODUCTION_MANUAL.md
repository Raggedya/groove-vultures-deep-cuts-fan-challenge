...
JookBox uses `editionType: jukebox` and model `jookbox/2` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. Its isolated old-school cabinet uses subtly weathered wood, aged chrome/gold and restrained period illumination, with the verified 16:9 featured YouTube video as the visual hero. The dynamic band name is an auto-fitting, single-line illuminated cabinet sign above the smaller JookBox model mark. The user-facing coin and slot sit above the video, Share is mounted as cabinet hardware beside one-line video metadata, and no duplicate plaque, visible helper or sound-toggle control appears.

Each JookBox edition starts from one artist-controlled Linktree and captures its current visible entries as a dated research snapshot, then independently verifies every destination that may be published. Search results are never buttons, and Linktree entries never define the public control set. The tactile deck renders only verified `Bandcamp`, `Spotify`, `Instagram`, `Band Website`, `Buy Music`, `Buy Merch` and `Facebook` controls, in that order. Missing entries are omitted; purchase controls require direct verified stores. The embedded video keeps its own playback controls, while YouTube, tickets, newsletter, TikTok, contact, podcast, reaction-video and all other Linktree categories remain absent from the destination deck. A user-pushed coin unlocks the approved destinations, initiates autoplay and plays the one-time coin sound while the cabinet lights up. Ambient motion is restrained, respects reduced motion and never interferes with keyboard focus or outbound navigation. `Learn More` is an inline cabinet selection after the verified deck, never a sticky bottom navigation strip, and uses separately verified official biography evidence.
