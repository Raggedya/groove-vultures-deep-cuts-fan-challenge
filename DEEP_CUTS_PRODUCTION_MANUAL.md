...
JookBox uses `editionType: jukebox` and locked model `jookbox/3` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. The Filthy Animals edition is the owner-approved visual reference: a full-height 762 × 1280 photoreal cabinet whose band sign, broad upper amber ticker, 16:9 YouTube screen, centred post-video coin mechanism, red Share button, brass biography dial and four rows of two selection keys remain in their approved proportions. Its supplied cabinet file is SHA-256 protected.

Each JookBox edition starts from one artist-controlled Linktree and captures its visible entries as dated research evidence, then independently verifies every destination that may be published. Runtime scraping never creates controls. An explicit ordered `displaySelectionIds` list may expose up to eight verified snapshot entries; unavailable or stale entries are omitted. Filthy Animals permanently uses the eight owner-approved ticket, newsletter and YouTube keys represented in the locked reference.

The separately versioned `six-key/1` format always presents six uniform keys in a three-by-two grid. It accepts four to six verified external destinations in configured order. Learn More fills the first unoccupied key and Share fills the next; fewer than four verified external destinations fail validation. The light advances gently through one key at a time in reading order. These rules change population and timing only: every approved edition retains its exact key shape, typography, alignment, spacing, colours, borders, dimensions and glow treatment.
...
Share and the sourced second-screen biography remain available through the cabinet's red button and brass dial in the classic format. When either utility occupies a `six-key/1` fallback position, only its duplicate standalone control is hidden. There is no generic primary action, sticky action bar, visible helper, duplicate plaque or sound toggle.
...
ATLAS `dc_e22f1cb651` has one isolated owner-approved presentation exception. Its `atlas-reference-cabinet/1` appearance uses a SHA-256-locked 887 × 1774 photoreal black-walnut, chrome and neon cabinet. Live HTML occupies its dedicated marquee, ticker, coin bay, YouTube screen, three-by-two key bank, share panel and lower status hardware. The large panel is the working `Support Our Band` share action, with `Please share our JookBox`, the existing heart and a small share icon. Its former lower Share hardware is replaced by the in-cabinet text `Copyright Clearlight Creative 2026.`; the immutable global `Deep Cuts` / `Copyright Clearlight Creative` footer remains unchanged beneath the cabinet. Only verified ATLAS destinations render; no website or merchandise key is invented. Its exact existing key shape, typography, alignment, spacing, colours, borders, dimensions and glow appearance are permanently locked. ATLAS uses `six-key/1`: five verified destinations retain their order, Learn More fills the sixth position, and the gentle light advances one key at a time from top-left to bottom-right. The Filthy Animals reference, factory default and all other completed products retain their original presentation.
