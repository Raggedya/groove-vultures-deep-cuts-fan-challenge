...
JookBox uses `editionType: jukebox` and model `jookbox/2` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. Its isolated old-school cabinet uses wood, chrome/gold and vivid period neon, keeps the verified featured YouTube video inside the cabinet and renders the artist-controlled Linktree as physical selection keys.

Each JookBox edition starts from one artist-controlled Linktree, captures its current visible buttons as a dated build-time snapshot, then independently verifies every published destination. Search results are never buttons. Buttons automatically align from the verified `jookBox.selections` array and missing entries are omitted. If an official YouTube channel exists, the embedded feature is its most-viewed embeddable official video at verification time. A user-pushed coin unlocks the buttons and initiates autoplay, sound and a short cabinet light-up sequence. Sequential lights are decorative only, respect reduced motion and never interfere with keyboard focus or outbound navigation. `Learn More` uses separately verified official biography evidence.
