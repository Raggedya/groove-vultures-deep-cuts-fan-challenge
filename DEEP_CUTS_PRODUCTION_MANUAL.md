...
The Laneway company edition additionally records intentional wheel spins and completions; selected artists with wheel, Surprise Me, roster, recommendation or quiz-result source; verified platform clicks; recommendation impressions and selections; anonymous directory searches; quiz starts, answers, completions, abandonments and replays; quiz-to-catalogue selections; services-contact intent; session duration; and distinct artists discovered per anonymous session. The Worker accepts only a versioned allow-list of scalar metadata. Duplicate event IDs are ignored by D1, reports reconcile against the raw event audit, every verified roster artist is included even when the result is zero, and report generation fails closed instead of silently truncating data.

After a Laneway company selection, the rich artist card reveals the sourced description, reason to listen, optional verified starting release and only the verified destinations present on that roster record. Each purchase destination must be a direct, identity-verified HTTPS artist or label-store page with dated evidence in the isolated roster; a search result, generic storefront, ambiguous identity, sold-out-only merch page or unavailable destination is omitted. All platform clicks use the existing trusted `artist_destination_clicked` event with artist, destination and discovery-source attribution. This rule is exclusive to the Laneway company edition and must not change artist-specific Laneway, Indie Wheel, Music, Cars, Clubs, Schools or Record Company editions.

The Laneway company page follows a permanent fan journey: branding; discovery heading and concise catalogue introduction; wheel; selected artist card; Surprise Me; curated related artists; searchable roster; ten-question catalogue quiz; licensing contact; footer. The quiz copy is `How Well Do You Know the Laneway Catalogue?`, `Ten questions from across Laneway Music history.` and `Take the Quiz`. It remains below search and core discovery rather than interrupting the wheel. The one-off uses restrained red attention within its black-and-charcoal identity; the former pale-blue heartbeat and delayed invitation are retired.

While the standalone Laneway company wheel is moving, its centre control replaces text with a compact generated white spiral that rotates entirely inside the circle. The control retains an accessible selecting label, changes from `Discover` to `Spin Again` after a result, and uses a brief selection path for reduced-motion visitors. This treatment is exclusive to `laneway_company`; Indie Wheel retains its existing text state.
