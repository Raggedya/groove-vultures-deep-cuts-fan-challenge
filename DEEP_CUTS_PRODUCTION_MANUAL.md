...
JookBox uses `editionType: jukebox` and locked model `jookbox/3` on the shared engine. It is a band-only discovery model with no Aggits, quiz or spinning wheel. The Filthy Animals edition is the owner-approved visual reference: a full-height 762 × 1280 photoreal cabinet whose band sign, coin mechanism, 16:9 YouTube screen, red Share button, amber ticker, brass biography dial and four rows of two selection keys remain in their approved proportions. Its supplied cabinet file is SHA-256 protected.
...
The cabinet starts in `sleeping`, accepts one accessible coin, plays a locally stored, sourced and licensed real coin-slot recording after that gesture, moves through `acceptingCoin` and `poweringUp`, then reaches `awake`. While sleeping, every selection key remains clearly dimmed, removed from keyboard navigation and unavailable; the coin interaction begins the staged illumination, and links unlock only at `awake`. Neon hesitates and settles, the screen uses a restrained CRT reveal, selection keys illuminate two at a time by row and the sourced uppercase biography travels right-to-left in natural reading order across a brighter, slightly larger amber ticker. The privacy-enhanced YouTube player requests playback synchronously inside the direct coin interaction, with no post-coin delay; the real YouTube play control remains visible because a browser may still block audible autoplay. The awake flag is edition-specific session state: a refresh restores the live cabinet without replaying sound, start-up or autoplay. Hidden tabs pause continuous animation; reduced motion replaces the theatrical sequence with a gentle fade, static biography and non-flashing controls. Missing audio logs a development warning and never blocks the visual sequence.
