...
Aggits Jukebox publication uses its own protected publisher, D1 records and R2 key prefix. `SAVE + PUBLISH JUKEBOX` validates the title, ticker, one-to-four enabled actions and public MP4 ceiling, assigns or reuses one stable opaque edition ID, uploads the versioned media, and generates the permanent QR from the deployed HTTPS URL. The QR renderer uses the SHA-256-locked 1254 x 1254 owner artwork, fits the edition title into the upper plaque, confines the QR to the bright opening, and scan-tests the completed artwork at full and phone-size resolution. The completion email names the edition, provides the permanent URL as both a clickable link and plain copyable text, and attaches the fitted QR image. Delivery confirmation plus live page, video and QR verification are mandatory before Studio reports success; failure rolls back without changing an existing live version.
