...
Bar Edition is the static `bar_jukebox` Studio workflow. It accepts one venue name, exactly five administrator-labelled HTTPS destinations, administrator ticker copy, administrator-approved About Us copy and one local MP4 up to 500 MB for local preview; production MP4 assets are fail-closed at 24 MiB. It performs no web lookup, accepts no YouTube link, and always supplies an internal About Us control as the sixth key. Inside the cabinet, the arch marquee shows only the curved, automatically fitted venue name and `JOOKBOX` tagline; it never displays `BAR EDITION`. The support strip below the keys is the sole working `Share [Venue Name] with your mates` control after coin activation. Its preview uses the locked heritage brass cabinet and the shared drag/click/keyboard coin, real coin sound, neon/CRT and ticker. Its six period SVG keys illuminate two at a time in three 1.5-second warm incandescent stages. The yellow coin pulses only before activation, then inserts and remains absent without pulse or glow while awake, including after an awake-session refresh. A direct coin gesture requests immediate MP4 playback; native controls remain the fallback. Because the private preview is bound to ephemeral `127.0.0.1`, Studio does not create or download a Bar Edition preview QR. On first production use, the owner activates the installed Windows Studio using a six-digit code sent to the configured owner email; the device token is encrypted by Windows. Publish Venue then validates locally and remotely, uploads the SHA-256-verified MP4 and locally generated two-size scan-tested QR to versioned R2 objects, activates the isolated D1 edition record, confirms completion-email delivery, and verifies the live page, MP4 and QR. The owner performs no GitHub or Cloudflare step, and a failed verification rolls back before the venue is marked published. See `BAR_EDITION_MODEL.md`.
