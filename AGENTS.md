...
`Bar Edition` uses the separately locked static `bar-jukebox/1` configuration. It reuses the ATLAS cabinet engine without altering Band Edition and accepts only administrator-supplied venue name, ticker, About Us copy, local MP4 and exactly five labelled HTTPS actions; About Us is the sixth key and the support strip is the sole working `Share [Venue Name] with your mates` control. Never run web lookup, infer missing content, add Aggits, add YouTube, add a quiz or add a wheel. Never encode the private ephemeral `127.0.0.1` preview in a Bar Edition QR; create and scan-test the permanent QR only after the public HTTPS deployment exists.
