...
Then read `DEEP_CUTS_PRODUCTION_MANUAL.md`, `CLAUDE.md`, `ROADMAP.md`, and `.agents/skills/deep-cuts-factory/SKILL.md` before acting.
...
`Bar Edition` uses the separately locked static `bar-jukebox/1` configuration. It reuses the ATLAS cabinet engine without altering Band Edition and accepts only administrator-supplied venue name, ticker, About Us copy, local MP4 and exactly five labelled HTTPS actions; About Us is the sixth key and the support strip is the sole working Share control, visibly labelled exactly `SHARE`. Never run web lookup, infer missing content, add Aggits, add YouTube, add a quiz or add a wheel. Never encode the private ephemeral `127.0.0.1` preview in a Bar Edition QR; create and scan-test the permanent QR only after the public HTTPS deployment exists.

Venue Library's **Secure Publish Venue** action is the sole local-to-production boundary for Bar Edition. The owner activates one Windows installation by email once; the device credential is scoped to that installation and encrypted with Electron `safeStorage`. A single Publish Venue action must validate the static venue, upload its versioned MP4 and scan-tested permanent QR to the private Cloudflare publisher, register the isolated edition, confirm delivery email, and verify the live page, QR and MP4 before changing publication state. The owner must never need GitHub, Cloudflare credentials or a manual deployment step. Live MP4 files are capped at 24 MiB; fail closed above that limit, failed or interrupted jobs remain unpublished, and rollback must preserve the previous live version.
