...
The isolated `jukebox` product type provides the reusable `jookbox/3` band model: no Aggits, quiz or spinning wheel; an owner-locked full-height photoreal cabinet; an accessible top-mounted coin; a sourced and licensed local real coin-slot recording; a four-state session-restored wake-up; restrained neon and CRT activation; a verified 16:9 YouTube screen that requests playback immediately within the coin interaction while retaining its browser-safe manual play fallback; red Share hardware; a brighter, slightly larger right-to-left amber sourced biography ticker; a brass biography dial; and up to eight explicit verified selection keys that remain dimmed and unavailable before the coin, then illuminate two at a time by row and unlock at `awake`. Linktree remains dated build-time evidence and only IDs named in `displaySelectionIds` can appear. Filthy Animals is the locked reference edition.
