...
The isolated `jukebox` product type provides the reusable `jookbox/3` band model: no Aggits, quiz or spinning wheel; an owner-locked full-height photoreal cabinet; an accessible top-mounted coin; local mechanical audio; a four-state session-restored wake-up; restrained neon and CRT activation; a user-controlled verified 16:9 YouTube screen; red Share hardware; an amber left-to-right sourced biography ticker; a brass biography dial; and up to eight explicit verified selection keys illuminating two at a time by row. Linktree remains dated build-time evidence and only IDs named in `displaySelectionIds` can appear. Filthy Animals is the locked reference edition.
