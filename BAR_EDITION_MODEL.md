...
The top marquee displays only the venue name and the `JOOKBOX` tagline; it never displays `BAR EDITION`. The editable venue name follows the engraved heritage arch on a responsive SVG path and is automatically fitted without flattening the curve. The broad cabinet ticker uses large, bright yellow type and remains one clipped scrolling line. During private publication, the Venue Library reads the configured official Gigs page and uses supported structured upcoming-event facts for that static ticker; administrator copy remains the fallback when no supported event is available. The administrator supplies approved About Us copy, a local MP4 welcome video and exactly five button labels with five direct HTTPS destinations. About Us is always the internal sixth parchment/brass key. The six physical keys use one coherent monochrome antique SVG icon set. While awake they illuminate individually in reading order; each warm incandescent lamp is clipped inside its parchment face and never casts a square outside the bronze frame. The cabinet support strip below the keys is the sole working Share control and its visible label is exactly `SHARE`. `Copyright Clearlight Creative 2026.` is displayed on its own brass plate at the bottom of the cabinet.
...
7. Select the local MP4 welcome video. The locked heritage aperture is portrait `1114 × 1205` pixels; use that exact Camtasia canvas and export size to fill the screen without matte bars or material cropping. For live publication, keep the exported MP4 at 24 MiB or smaller.
