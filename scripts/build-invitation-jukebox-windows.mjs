...
const root = process.cwd(),
  dependencyRoot = process.env.DEEP_CUTS_DEPENDENCY_ROOT || root,
...
const version = "1.0.0",
  ...
  outputDirectory = path.join(
    root,
    "output",
    `Invitation-Jukebox-Windows-${version}`,
  ),
  temporaryRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "invitation-jukebox-windows-"),
  ),
  sourceDirectory = path.join(temporaryRoot, "source"),
  packagedDirectory = path.join(temporaryRoot, "packaged");
const runtimeFiles = [
  "studio/invitation-desktop-main.mjs",
  "invitation-studio/index.html",
  "invitation-studio/styles.css",
  "invitation-studio/app.js",
  "scripts/invitation-studio-server.mjs",
  "scripts/invitation-jukebox-model.mjs",
  "scripts/invitation-jukebox-preview.mjs",
  "scripts/invitation-jukebox-publication.mjs",
  "scripts/invitation-jukebox-qr-artwork.mjs",
  ...
  "assets/invitation-jukebox/wedding-cabinet.png",
  "assets/invitation-jukebox/birthday-cabinet.png",
  "assets/invitation-jukebox/corporate-cabinet.png",
  "assets/invitation-jukebox/seasonal-cabinet.png",
  "assets/invitation-jukebox/group-trip-cabinet.png",
  "assets/jookbox-venue-qr-master-v1.png",
];
const log = (text) => console.log(`[Invitation Jukebox] ${text}`);
...
async function electronZip() {
  for (const directory of [
    ...
    path.join(dependencyRoot, ".tools", "localappdata", "electron", "Cache"),
...
async function installer(appDirectory) {
  const portableName = `Invitation-Jukebox-${version}-Windows-x64.zip`,
  ...
  const ps = [
    ...
    "if(-not (Test-Path -LiteralPath $application)){throw 'Invitation Jukebox executable was not installed.'}",
    "$shell=New-Object -ComObject WScript.Shell",
    "function Set-Shortcut([string]$where){$shortcut=$shell.CreateShortcut($where);$shortcut.TargetPath=$application;$shortcut.WorkingDirectory=$installDirectory;$shortcut.Description='Invitation Jukebox';$shortcut.Save()}",
    `Set-Shortcut (Join-Path ([Environment]::GetFolderPath('Desktop')) '${productName}.lnk')`,
    "$menu=Join-Path $env:APPDATA 'Microsoft\\Windows\\Start Menu\\Programs\\Invitation Jukebox'",
...
if (process.platform !== "win32")
  throw new Error("Build the Invitation Jukebox installer on Windows.");
try {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify({ name: "invitation-jukebox", productName, type: "module", private: true, version, description: "Custom invitation creator with typed libraries, permanent publishing, QR artwork and email delivery.", main: "studio/invitation-desktop-main.mjs", author: "Clearlight Creative", dependencies: { "electron-squirrel-startup": "^1.0.1", sharp: "0.35.2", yauzl: "2.10.0" } }, null, 2)}\n`,
  );
  ...
  for (const moduleName of [
    ...
    await fs.cp(
      path.join(dependencyRoot, "node_modules", ...moduleName.split("/")),
