...
const deliveryRecovery=await fs.readFile(".github/workflows/record-company-delivery-recovery.yml","utf8");
...
for(const route of ["/api/record-company/","/record-company/q/","/record-company/"])assert.ok(workerIndex.includes(route));
...
for(const text of ["record_company_url","generate-deliverables.py","send-completion.mjs","git rebase origin/main","git push origin HEAD:main","generating_master_qr_image"])assert.ok(workflow.includes(text));
...
for(const recoveryMarker of ["Record Company Delivery:","send-completion.mjs","record-company-output/$slug","Close completed delivery request"])assert.ok(deliveryRecovery.includes(recoveryMarker));
