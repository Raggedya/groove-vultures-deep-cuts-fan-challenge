...
const tracker=new A.Tracker({platformConfig:{analytics:{localRetention:2500}},editionEntry:{slug:'band-one'},editionConfig:{bandName:'Band One'},storage,windowObject:{dispatchEvent(){}},documentObject:{referrer:'',head:null},navigatorObject:{userAgent:'iPhone'},locationObject:{search:'?utm_source=poster'},now:()=>new Date(now)});
const viewed=tracker.track('discovery_page_viewed',{}, {onceKey:'page:band-one'});assert.equal(viewed.quiz_identifier,'band-one:discovery-v1');assert.equal(viewed.referring_source,'poster');assert.equal(tracker.track('discovery_page_viewed',{}, {onceKey:'page:band-one'}),null);
