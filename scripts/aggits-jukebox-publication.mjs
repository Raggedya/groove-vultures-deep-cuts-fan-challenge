...
export async function verifyPublicEdition(fetchImpl,job,title){const {MAHOGANY_RENDERER_VERSION}=await import("./aggits-jukebox-preview.mjs"),origin=new URL(job.liveUrl).origin,[page,config,qr,video]=await Promise.all([fetchImpl(`${job.liveUrl}?publication=${job.id}`,{cache:"no-store"}),fetchImpl(`${origin}/api/aggits-jukebox-editions/${job.editionId}/config`,{cache:"no-store"}),fetchImpl(job.qrImageUrl,{cache:"no-store"}),fetchImpl(`${origin}/api/aggits-jukebox-assets/${job.editionId}/video`,{method:"HEAD",cache:"no-store"})]);const [html,json,qrBytes]=await Promise.all([page.text(),config.json().catch(()=>null),qr.arrayBuffer()]),png=new Uint8Array(qrBytes);if(!page.ok||!html.includes(`content="${MAHOGANY_RENDERER_VERSION}"`)||page.headers.get("x-deep-cuts-renderer")!==MAHOGANY_RENDERER_VERSION)throw publicationError("The public service is not running the accepted Mahogany Jukebox graphics.","live_verification_failed");if(!config.ok||json?.editionType!=="aggits_jukebox"||json?.bandName!==title||json?.aggitsJukebox?.modelVersion!==MAHOGANY_RENDERER_VERSION)throw publicationError("The live Jukebox configuration did not match this edition.","config_verification_failed");if(!qr.ok||png.length<10000||png[0]!==0x89||png[1]!==0x50)throw publicationError("The deployed QR artwork failed verification.","qr_verification_failed");if(!video.ok||!(video.headers.get("content-type")||"").includes("video/mp4"))throw publicationError("The deployed MP4 failed verification.","video_verification_failed")}
