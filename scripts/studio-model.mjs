...
function renderBarJookBoxStudioIcon(label){
  ...
  const paths={
    ...
    instagram:'<rect x="11" y="11" width="58" height="58" rx="15"/><circle cx="40" cy="40" r="14"/><circle cx="59" cy="21" r="3" class="fill"/>',
    facebook:'<path class="fill" d="M46 70V44h9l2-11H46v-7c0-4 2-7 7-7h6V9c-3-1-7-1-11-1-11 0-18 7-18 19v6H20v11h10v26Z"/>',
    about:'<circle cx="40" cy="40" r="30"/><path d="M40 34v22"/><circle cx="40" cy="23" r="2.5" class="fill"/>',
  ...
  return`<svg class="key-icon key-icon-${type}" viewBox="0 0 80 80" aria-hidden="true"><g>${paths[type]}</g></svg>`;
}

function renderBarStudioShareHand(side){
  return`<svg class="share-hand share-hand-${side}" viewBox="0 0 128 58" aria-hidden="true"><path d="M5 16h20v29H5ZM10 21h9M10 27h9M10 33h9M10 39h9"/><path d="M25 20h9c5 0 8 3 11 7l4 5 7-2 9-9c3-3 8-3 10 0 2 3 1 6-2 9l-4 4 9-5c4-2 8-1 10 2 1 3 0 6-3 8l-4 2 9-1c4 0 7 2 7 5 0 4-3 6-7 6H54c-6 0-10-2-15-5l-14-8Z"/><path d="M88 40h33c2 0 4 2 4 4s-2 4-4 4H88"/></svg>`;
}
...
<head>
  ...
  <style>
    *{box-sizing:border-box}[hidden]{display:none!important}html{background:#050201}body{margin:0;min-height:100vh;background:#050201;color:#fff;font-family:Arial,Helvetica,sans-serif}
    ...
    .title{position:absolute;z-index:3;top:6.15%;left:14%;width:72%;height:12.7%;color:#d8b36c;filter:brightness(.56) saturate(.62);transition:filter .45s}.machine.is-neon-on .title{filter:brightness(1) saturate(.9)}.title h1{position:absolute;width:1px;height:1px;padding:0;overflow:hidden;clip:rect(0 0 0 0);clip-path:inset(50%);white-space:nowrap}.title svg{display:block;width:100%;height:100%;overflow:visible}.title-curve{fill:#d3b074;stroke:rgba(48,25,11,.74);stroke-width:1.35;paint-order:stroke fill;font:800 88px/1 Georgia,serif;letter-spacing:.045em;filter:drop-shadow(0 2px 0 #2a1308) drop-shadow(0 0 2px rgba(248,219,155,.24))}.title-model{fill:#d1ad70;stroke:rgba(45,23,9,.68);stroke-width:.7;paint-order:stroke fill;font:800 25px/1 Georgia,serif;letter-spacing:5px;filter:drop-shadow(0 1px 0 #2b1508)}.title-ornament{fill:none;stroke:#a57b43;stroke-width:1.7;stroke-linecap:round;stroke-linejoin:round;opacity:.82;filter:drop-shadow(0 1px 0 #251006)}
    ...
    .screen{position:absolute;z-index:3;top:29.75%;left:31.1%;width:57.5%;height:35.55%;padding:1.15%;overflow:hidden;border:0;border-radius:5px;background:#020202;box-shadow:inset 0 0 12px #000}.screen:after{position:absolute;z-index:4;inset:0;content:"";pointer-events:none;background:#010101;opacity:.84;transition:opacity .42s}.machine.is-screen-on .screen:after{opacity:0}.screen video{display:block;width:100%;height:100%;object-fit:contain;background:#000;pointer-events:none;filter:brightness(.28) saturate(.42);transition:filter .42s}.machine.is-screen-on .screen video{pointer-events:auto;filter:none}.video-wait{height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:18px;color:#b99558;text-align:center}.video-wait strong{font-size:12px;letter-spacing:.08em}.video-wait span{max-width:230px;margin-top:7px;color:#806c52;font-size:8px;line-height:1.4}
    .keys{position:absolute;z-index:4;top:66.15%;left:11.85%;width:76.85%;height:21.05%;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(2,1fr);gap:4.35% 2.15%}.key{min-width:0;display:flex;align-items:center;flex-direction:column;gap:2%;justify-content:center;padding:5% 4%;border:0;border-radius:4%;background:linear-gradient(180deg,rgba(255,234,181,.012),rgba(90,46,17,.018));color:#2b180c;text-decoration:none;text-align:center;box-shadow:inset 0 1px rgba(255,244,210,.025),inset 0 -3px rgba(55,26,9,.025);filter:brightness(.3) saturate(.34);opacity:.34;pointer-events:none;transition:filter .42s,opacity .42s,box-shadow .42s,background .42s,transform .1s}.key-icon-frame{display:grid;width:100%;height:54%;place-items:center}.key-icon{width:min(59%,64px);height:100%;overflow:visible;color:inherit;filter:drop-shadow(0 1px 0 rgba(255,235,188,.22))}.key-icon g{fill:none;stroke:currentColor;stroke-width:3.2;stroke-linecap:round;stroke-linejoin:round}.key-icon .fill{fill:currentColor;stroke:none}.key strong{font:800 clamp(9px,2.43vw,12px)/1 Georgia,serif;letter-spacing:.01em;overflow-wrap:normal;text-transform:uppercase}.key.is-placeholder{opacity:.18}.machine.is-buttons-on .key:not(.is-placeholder){color:#2a180c;filter:brightness(.92) saturate(.79);opacity:1;pointer-events:auto}.machine.is-buttons-on .key.is-current{color:#211108;background:radial-gradient(ellipse 78% 72% at 50% 42%,rgba(255,247,207,.31),rgba(255,221,145,.12) 51%,transparent 76%);filter:brightness(1.13) saturate(.84);box-shadow:inset 0 0 12px rgba(255,252,222,.52),inset 0 -5px 12px rgba(188,106,35,.2),0 0 7px rgba(255,205,104,.26);animation:keyGlow 1500ms ease-in-out both}.machine.is-buttons-on .key:focus-visible{outline:2px solid #fff1bd;outline-offset:-3px}.machine.is-buttons-on .key:active{transform:translateY(2px)}
    .support{position:absolute;z-index:4;top:87.7%;left:12.05%;width:76.55%;height:7.75%;display:grid;place-content:center;padding:1.05% 15.5%;border:0;border-radius:2%;background:linear-gradient(180deg,rgba(255,240,199,.018),rgba(103,54,21,.025));color:#28160b;font:inherit;text-align:center;box-shadow:inset 0 1px rgba(255,245,215,.045),inset 0 -3px rgba(50,24,8,.045);filter:brightness(.28) saturate(.38);opacity:.48;pointer-events:none;transition:filter .3s,opacity .3s,box-shadow .3s,transform .1s}.share-hand{position:absolute;top:50%;display:block;width:13%;height:auto;fill:none;stroke:#2b180c;stroke-width:2.25;stroke-linecap:round;stroke-linejoin:round;filter:drop-shadow(0 1px rgba(255,235,181,.2));transform:translateY(-50%)}.share-hand-left{left:4.2%}.share-hand-right{right:4.2%;transform:translateY(-50%) scaleX(-1)}.support strong{display:grid;gap:.12em;place-items:center;color:#28160b;font:800 clamp(10px,2.85vw,15px)/.98 Georgia,serif;letter-spacing:.015em;text-transform:uppercase;text-wrap:balance}.support strong span:last-child{display:flex;width:100%;align-items:center;justify-content:center;gap:4%;font-size:.8em;letter-spacing:.045em;white-space:nowrap}.support strong span:last-child:before,.support strong span:last-child:after{width:14%;height:1px;content:"";background:linear-gradient(90deg,transparent,#5e3518)}.support strong span:last-child:after{background:linear-gradient(90deg,#5e3518,transparent)}.machine.is-buttons-on .support{filter:brightness(.98) saturate(.78);opacity:1;pointer-events:auto;cursor:pointer;box-shadow:inset 0 0 14px rgba(255,235,185,.13),inset 0 -3px rgba(66,31,9,.08)}.machine.is-buttons-on .support:focus-visible{outline:2px solid #fff0b0;outline-offset:-4px;filter:brightness(1.08)}.machine.is-buttons-on .support:active{transform:translateY(2px)}
    .status{display:none}.copyright{position:absolute;z-index:4;top:96.05%;left:31%;display:grid;width:38%;height:2.7%;padding:0 2%;place-items:center;overflow:hidden;border:1px solid #633919;border-radius:5%;color:#2c180c;background:linear-gradient(145deg,#c69d58,#87602f 48%,#c29a56);box-shadow:inset 0 1px rgba(255,238,184,.45),inset 0 -2px 5px rgba(55,27,7,.38),0 2px 5px rgba(0,0,0,.76);font:700 clamp(6px,1.45vw,9px)/1.05 Georgia,serif;letter-spacing:.025em;text-align:center}
...
<body>
    ...
    <section id="machine" class="machine" aria-label="${escapeAttribute(input.name||"Venue")} Bar Edition preview">
      <div class="title">
        ...
        <svg viewBox="0 0 720 222" preserveAspectRatio="xMidYMid meet" aria-hidden="true">
          <defs><path id="studioBarTitlePath" d="M62 118Q360 6 658 118"/></defs>
      ...
      <button id="sharePanel" class="support" type="button" aria-label="Share ${escapeAttribute(input.name||"this venue")} with your mates" aria-disabled="true" disabled>${renderBarStudioShareHand("left")}<strong><span>Share ${escapeHtml(input.name||"this venue")}</span><span>With your mates</span></strong>${renderBarStudioShareHand("right")}</button>
  ...
  <script${scriptNonce?` nonce="${escapeAttribute(scriptNonce)}"`:""}>
    ...
    const sequence=()=>{clearTimeout(keyTimer);if(document.hidden||!machine.classList.contains("is-buttons-on")||matchMedia("(prefers-reduced-motion: reduce)").matches||!keys.length)return;keyIndex=(keyIndex+1)%Math.ceil(keys.length/2);keys.forEach((key,index)=>key.classList.toggle("is-current",Math.floor(index/2)===keyIndex));keyTimer=setTimeout(sequence,1500)};
    ...
    const title=document.getElementById("barTitle"),frame=title?.closest(".title");let pending=0;const fit=()=>{pending=0;title.removeAttribute("textLength");title.removeAttribute("lengthAdjust");title.style.fontSize="88px";const target=596,measured=Math.max(1,title.getComputedTextLength?.()||target),size=Math.max(35,Math.min(88,88*target/measured));title.style.fontSize=size.toFixed(2)+"px";if((title.getComputedTextLength?.()||target)>target){title.setAttribute("textLength",String(target));title.setAttribute("lengthAdjust","spacingAndGlyphs")}};const schedule=()=>{cancelAnimationFrame(pending);pending=requestAnimationFrame(fit)};new ResizeObserver(schedule).observe(frame);document.fonts?.ready?.then(schedule).catch(()=>{});schedule()})();
  </script>
</body>
</html>`;
}
