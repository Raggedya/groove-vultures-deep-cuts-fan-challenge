...
<body>
  ...
  <script${scriptNonce?` nonce="${escapeAttribute(scriptNonce)}"`:""}>
    ...
    const sequence=()=>{clearTimeout(keyTimer);if(document.hidden||!machine.classList.contains("is-buttons-on")||matchMedia("(prefers-reduced-motion: reduce)").matches||!keys.length)return;keyIndex=(keyIndex+1)%keys.length;keys.forEach((key,index)=>key.classList.toggle("is-current",index===keyIndex));keyTimer=setTimeout(sequence,1100)};
