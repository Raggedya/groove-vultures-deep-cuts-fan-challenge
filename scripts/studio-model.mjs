...
function renderBarJookBoxStudioPreview(project,{videoUrl="",scriptNonce=""}={}){
  ...
  const keys=Array.from({length:5},(_,index)=>{
    ...
    const icon=/contact/.test(identity)?"&#9742;":/instagram/.test(identity)?"&#9678;":/facebook/.test(identity)?"f":/gig|show|event/.test(identity)?"&#9835;":/menu|drink/.test(identity)?"&#10087;":/website/.test(identity)?"&#8962;":"&#10022;";
    return item
      ?`<a class="key" href="${escapeAttribute(item.url)}" target="_blank" rel="noopener noreferrer" aria-disabled="true" tabindex="-1"><span aria-hidden="true">${icon}</span><strong>${escapeHtml(item.label)}</strong></a>`
      :`<span class="key is-placeholder" aria-hidden="true"><strong>BUTTON ${index+1}</strong></span>`;
  });
  keys.push('<button id="aboutKey" class="key is-about" type="button" aria-disabled="true" disabled><span aria-hidden="true">&#9432;</span><strong>ABOUT US</strong></button>');
...
<head>
  ...
  <style>
    *{box-sizing:border-box}[hidden]{display:none!important}html{background:#050201}body{margin:0;min-height:100vh;background:#050201;color:#fff;font-family:Arial,Helvetica,sans-serif}
    ...
    .title{position:absolute;z-index:3;top:6.15%;left:14%;width:72%;height:12.7%;display:grid;align-content:center;color:#d8b36c;font-family:Georgia,serif;font-weight:900;line-height:.88;letter-spacing:.075em;text-align:center;text-shadow:0 1px #3a1c08,0 0 3px rgba(255,224,153,.38);text-transform:uppercase}
    .title>strong{display:block;width:100%;overflow:hidden;font:inherit;font-size:clamp(22px,7vw,42px);line-height:.88;text-overflow:clip;white-space:nowrap}.title>strong[data-fit-mode="multi-line"]{overflow-wrap:anywhere;text-wrap:balance;white-space:normal}.title small{display:block;margin-top:7px;color:#d5b06f;font:800 clamp(7px,2vw,12px) Georgia,serif;letter-spacing:.18em}
    ...
    .keys{position:absolute;z-index:4;top:66.15%;left:11.85%;width:76.85%;height:21.05%;display:grid;grid-template-columns:repeat(3,1fr);grid-template-rows:repeat(2,1fr);gap:4.35% 2.15%}.key{min-width:0;display:flex;align-items:center;flex-direction:column;gap:4%;justify-content:center;padding:8% 5% 6%;border:0;border-radius:4%;background:transparent;color:#2d1a0e;text-decoration:none;text-align:center;box-shadow:none;filter:brightness(.3) saturate(.35);opacity:.35;pointer-events:none;transition:filter .22s,opacity .22s,box-shadow .22s,transform .1s}.key span{display:grid;height:48%;place-items:center;font:900 clamp(16px,4.8vw,25px)/1 Georgia,serif}.key strong{font:900 clamp(8px,2.35vw,12px)/1.02 Georgia,serif;overflow-wrap:anywhere;text-transform:uppercase}.key.is-placeholder{opacity:.18}.machine.is-buttons-on .key:not(.is-placeholder){color:#2d1a0e;filter:brightness(.92) saturate(.82);opacity:1;pointer-events:auto}.machine.is-buttons-on .key.is-current{color:#211208;filter:brightness(1.12) saturate(.9);box-shadow:inset 0 0 18px rgba(255,238,187,.46),0 0 8px rgba(255,207,112,.42)}.machine.is-buttons-on .key:focus-visible{outline:2px solid #fff1bd;outline-offset:-3px}.machine.is-buttons-on .key:active{transform:translateY(2px)}
    .support{position:absolute;z-index:4;top:87.7%;left:12.05%;width:76.55%;height:7.75%;display:grid;place-content:center;padding:2% 14%;border:0;border-radius:2%;background:transparent;color:#2c190d;font:inherit;text-align:center;box-shadow:none;filter:brightness(.28) saturate(.38);opacity:.48;pointer-events:none;transition:filter .3s,opacity .3s,box-shadow .3s,transform .1s}.support:before,.support:after{position:absolute;top:50%;font:900 clamp(17px,5vw,27px)/1 Georgia,serif;transform:translateY(-50%)}.support:before{left:7%;content:"\\261e"}.support:after{right:7%;content:"\\261c"}.support strong{font:900 clamp(10px,3vw,16px)/1.08 Georgia,serif;letter-spacing:.02em;text-transform:uppercase}.machine.is-buttons-on .support{filter:brightness(.96) saturate(.8);opacity:1;pointer-events:auto;cursor:pointer;box-shadow:inset 0 0 14px rgba(255,235,185,.16)}.machine.is-buttons-on .support:focus-visible{outline:2px solid #fff0b0;outline-offset:-4px;filter:brightness(1.08)}.machine.is-buttons-on .support:active{transform:translateY(2px)}
