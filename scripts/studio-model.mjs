...
<body>
  ...
  <script${scriptNonce?` nonce="${escapeAttribute(scriptNonce)}"`:""}>
    ...
    const title=document.getElementById("barTitle"),frame=title?.closest(".title");let pending=0;const fit=()=>{pending=0;const characters=String(title.textContent||"").trim().length;title.removeAttribute("textLength");title.removeAttribute("lengthAdjust");title.style.letterSpacing=characters>22?".012em":characters>18?".018em":characters>14?".026em":".04em";title.style.fontSize="82px";const target=610,measured=Math.max(1,title.getComputedTextLength?.()||target),size=Math.max(33,Math.min(82,82*target/measured));title.style.fontSize=size.toFixed(2)+"px";const finalWidth=Math.max(1,title.getComputedTextLength?.()||target);if(finalWidth>target){title.setAttribute("textLength",String(target));title.setAttribute("lengthAdjust","spacingAndGlyphs")}title.dataset.fittedSize=size.toFixed(2);title.dataset.fittedWidth=Math.min(finalWidth,target).toFixed(2)};const schedule=()=>{cancelAnimationFrame(pending);pending=requestAnimationFrame(fit)};new ResizeObserver(schedule).observe(frame);document.fonts?.ready?.then(schedule).catch(()=>{});schedule()})();
  </script>
</body>
</html>`;
}
