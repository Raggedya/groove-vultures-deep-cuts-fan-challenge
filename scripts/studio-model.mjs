...
<body>
  ...
  <script${scriptNonce?` nonce="${escapeAttribute(scriptNonce)}"`:""}>
    ...
    const awaken=(restore=false)=>{if(state!=="sleeping")return;state="starting";machine.classList.add("is-accepting");status.textContent="Coin accepted — Bar Edition is powering up";if(!restore){const startVideo=()=>{if(video){video.currentTime=0;video.play().catch(()=>{status.textContent="Coin accepted — press play for the welcome video"})}};const coinPlayback=sound?.play();if(coinPlayback?.then)coinPlayback.then(startVideo).catch(error=>{console.warn("Coin recording could not be played.",error);startVideo()});else startVideo();try{sessionStorage.setItem("${sessionKey}","true")}catch{}}setTimeout(()=>machine.classList.add("is-powering"),120);setTimeout(()=>machine.classList.add("is-neon-on"),800);setTimeout(()=>machine.classList.add("is-screen-on"),1200);setTimeout(()=>{machine.classList.add("is-buttons-on");unlock();sequence()},1600);setTimeout(()=>{machine.classList.add("is-ticker-on","is-awake");machine.classList.remove("is-powering","is-accepting");state="awake";status.textContent="Coin accepted — JookBox is live"},2000)};
