...
<head>
  ...
  <style>
    *{box-sizing:border-box}html{background:#050201}body{margin:0;min-height:100vh;background:#050201;color:#fff;font-family:Arial,Helvetica,sans-serif}
    ...
    .title{position:absolute;z-index:3;top:4.15%;left:17%;width:66%;height:12.3%;display:grid;align-content:center;color:#ffe9a8;font-family:Georgia,serif;font-weight:900;line-height:.9;letter-spacing:.1em;text-align:center;text-shadow:0 0 3px #fff2b9,0 0 12px #ff9f22;text-transform:uppercase}
    .title>strong{display:block;width:100%;overflow:hidden;font:inherit;font-size:clamp(22px,8vw,42px);line-height:.9;text-overflow:clip;white-space:nowrap}.title>strong[data-fit-mode="multi-line"]{overflow-wrap:anywhere;text-wrap:balance;white-space:normal}.title small{display:block;margin-top:8px;color:#ffe0a0;font:800 clamp(7px,2.3vw,12px) Arial,sans-serif;letter-spacing:.3em}.title small:after{display:block;margin-top:5px;color:#ffb6cb;content:"BAR EDITION";font-size:.82em;letter-spacing:.14em;text-shadow:0 0 7px rgba(255,67,135,.72)}
...
<body>
  ...
  <script${scriptNonce?` nonce="${escapeAttribute(scriptNonce)}"`:""}>
    (()=>{const machine=document.getElementById("machine"),coinButton=document.getElementById("coinButton"),coin=document.getElementById("coin"),video=document.getElementById("welcomeVideo"),status=document.getElementById("status"),keys=[...document.querySelectorAll(".key:not(.is-placeholder)")],share=document.getElementById("shareKey"),sound=document.getElementById("coinSound");let state="sleeping",drag=null,keyTimer=0,keyIndex=-1;
    const unlock=()=>{keys.forEach(key=>{if(key.tagName==="A"){key.removeAttribute("aria-disabled");key.tabIndex=0}else{key.disabled=false;key.removeAttribute("aria-disabled")}})};
    ...
    document.querySelectorAll(".key").forEach(key=>key.addEventListener("click",event=>{if(state!=="awake"){event.preventDefault();coinButton.focus()}}));share?.addEventListener("click",async()=>{const payload={title:${JSON.stringify(`${input.name||"Venue"} | Bar Edition`)},text:${JSON.stringify(`Explore the ${input.name||"venue"} Bar Edition.`)},url:location.href};if(navigator.share){try{await navigator.share(payload);return}catch{}}try{await navigator.clipboard.writeText(location.href);status.textContent="Bar Edition link copied"}catch{status.textContent="Use your browser Share command"}});
