...
const jobId=process.env.DEEP_CUTS_DELIVERY_JOB_ID||config.production?.jobId||`delivery-${edition.editionId}-${Date.now()}`;
...
if(config.production?.submittedAt)await post('/api/builds',{job_id:jobId,edition_id:edition.editionId,band_name:edition.name,stage:'research_started',timestamp:config.production.submittedAt});
if(config.production?.researchCompletedAt)await post('/api/builds',{job_id:jobId,edition_id:edition.editionId,band_name:edition.name,stage:'research_completed',timestamp:config.production.researchCompletedAt});
await post('/api/builds',{job_id:jobId,edition_id:edition.editionId,band_name:edition.name,stage:'artwork_completed',timestamp:new Date().toISOString()});
await post('/api/builds',{job_id:jobId,edition_id:edition.editionId,band_name:edition.name,stage:'validation_completed',timestamp:new Date().toISOString()});
...
const confirmation=await waitForDelivery(jobId);
console.log(JSON.stringify({ok:true,job_id:jobId,email_id:delivery.email_id,email_delivered_at:confirmation.email_delivered_at,live_url:liveURL,qr_image_url:qrImageURL},null,2));
...
async function waitForDelivery(id){
  for(let attempt=0;attempt<30;attempt++){
    ...
    const result=await response.json().catch(()=>({}));
    if(response.ok&&result.email_delivered_at)return result;
