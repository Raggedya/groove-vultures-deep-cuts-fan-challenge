...
export async function createAggitsJukeboxQrArtwork({root,title,destination}={}){
  ...
  return{schemaVersion:AGGITS_JUKEBOX_QR_SCHEMA,bytes:png,sha256:crypto.createHash("sha256").update(png).digest("hex"),destination:payload,width:AGGITS_JUKEBOX_QR_SIZE,height:AGGITS_JUKEBOX_QR_SIZE,scanProof:"perspective-matrix:1254+627;decoder:360-required"};
}
...
function qrPlacement(count){
  const border=4,total=count+border*2;
  if(total>109)throw qrError("The permanent URL is too long for the locked QR panel.","qr_payload_too_long");
  return{border,total,panel:AGGITS_JUKEBOX_QR_PANEL};
}
function projectPoint(panel,u,v){
  const top={x:panel.topLeft.x+(panel.topRight.x-panel.topLeft.x)*u,y:panel.topLeft.y+(panel.topRight.y-panel.topLeft.y)*u};
  const bottom={x:panel.bottomLeft.x+(panel.bottomRight.x-panel.bottomLeft.x)*u,y:panel.bottomLeft.y+(panel.bottomRight.y-panel.bottomLeft.y)*u};
...
function renderOverlay(matrix,p,title){
  const modules=[];for(let row=0;row<matrix.length;row++)for(let column=0;column<matrix.length;column++)if(matrix[row][column]){
    const left=(column+p.border)/p.total,right=(column+p.border+1)/p.total,top=(row+p.border)/p.total,bottom=(row+p.border+1)/p.total;
    modules.push(`<polygon points="${polygon([projectPoint(p.panel,left,top),projectPoint(p.panel,right,top),projectPoint(p.panel,right,bottom),projectPoint(p.panel,left,bottom)])}" fill="#050403"/>`);
  }
...
async function verifyMatrix(png,matrix,p,scale){const {data,info}=await sharp(png).resize(Math.round(AGGITS_JUKEBOX_QR_SIZE*scale),Math.round(AGGITS_JUKEBOX_QR_SIZE*scale),{kernel:sharp.kernel.nearest}).removeAlpha().raw().toBuffer({resolveWithObject:true});let mismatches=0,total=0;for(let row=0;row<matrix.length;row++)for(let column=0;column<matrix.length;column++){const point=projectPoint(p.panel,(column+p.border+.5)/p.total,(row+p.border+.5)/p.total),x=Math.round(point.x*scale),y=Math.round(point.y*scale),i=(Math.min(info.height-1,y)*info.width+Math.min(info.width-1,x))*info.channels,dark=(data[i]+data[i+1]+data[i+2])/3<128;if(dark!==matrix[row][column])mismatches++;total++}const tolerance=scale<.4?.02:.002;if(mismatches>Math.max(1,Math.floor(total*tolerance)))throw qrError(`The ${info.width} x ${info.height} QR matrix-integrity check failed.`,"qr_scanback_failed")}
