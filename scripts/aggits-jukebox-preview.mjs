...
export function renderAggitsJukeboxStudioPreview(
  ...
  const buttons = Array.from({ length: 4 }, (_, index) => {
    ...
    const labelLength = String(action.label || "").trim().length;
    const labelClass =
      labelLength > 18
        ? " is-label-long"
        : labelLength > 11
    ...
    if (!enabled)
      return `<span class="action is-disabled${labelClass}" data-slot="${index + 1}" aria-hidden="true">${content}</span>`;
    const external = /^https:/i.test(action.href) && action.openInNewTab;
    return `<a class="action${labelClass}" data-slot="${index + 1}" data-enabled="true" data-href="${esc(action.href)}" ${external ? 'data-new-tab="true"' : ""} aria-disabled="true" tabindex="-1" aria-label="${esc(action.label)}${external ? " (opens in a new tab)" : ""}">${content}</a>`;
  }).join("");
...
<head>
  ...
  <style>
    *{box-sizing:border-box}[hidden]{display:none!important}html{background:#030201}body{margin:0;min-height:100vh;background:#030201;color:#f4d99c;font-family:Georgia,"Times New Roman",serif}button,a{font:inherit}
    ...
    .actions{position:absolute;z-index:4;top:56.45%;left:11.1%;width:78.9%;height:16.9%;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:1.25%;padding:.3% 0 0;overflow:hidden;background:#070402}.action{position:relative;isolation:isolate;display:grid;grid-template-rows:minmax(0,1fr) auto;place-items:center;min-width:0;min-height:0;padding:13% 5% 10%;overflow:hidden;border:1px solid #91612c;border-radius:50%;background:radial-gradient(ellipse at 48% 32%,#2a1b10 0,#0d0906 49%,#030201 82%);color:#d5a355;text-decoration:none;text-align:center;filter:brightness(.35) saturate(.55);opacity:.7;pointer-events:none;box-shadow:inset 0 0 0 2px #120b06,inset 0 0 0 4px #714921,inset 0 8px 13px rgba(199,133,58,.12),inset 0 -12px 16px #000;transition:filter .3s ease,opacity .3s ease,transform .14s cubic-bezier(.2,.7,.2,1),box-shadow .14s ease}.action:before{position:absolute;z-index:-1;inset:4%;border:1px solid #3d2915;border-radius:50%;content:"";box-shadow:inset 0 1px 1px rgba(255,211,123,.18),inset 0 -3px 5px #000}.action-icon{position:relative;display:block;width:62%;aspect-ratio:1;overflow:visible}.action-icon img{position:absolute;top:50%;left:50%;display:block;width:100%;height:100%;max-width:none;object-fit:contain;transform:translate(-50%,-50%)}.action strong{position:relative;z-index:1;display:grid;align-items:center;width:100%;max-width:100%;min-height:2.1em;max-height:3.15em;overflow:hidden;color:#c58a40;font:800 clamp(8px,3vw,14px)/1 Georgia,"Times New Roman",serif;letter-spacing:-.025em;text-transform:uppercase;text-shadow:0 1px 1px #000;overflow-wrap:anywhere;word-break:normal;text-wrap:balance}.action.is-label-medium strong{font-size:clamp(7.5px,2.65vw,12.5px)}.action.is-label-long strong{font-size:clamp(7px,2.25vw,10.5px)}.machine.is-buttons-ready .action[data-enabled="true"]{filter:brightness(.96) saturate(.9);opacity:1;pointer-events:auto}.action:focus-visible{outline:2px solid #ffe9ae;outline-offset:-4px}.action.is-depressed{transform:perspective(180px) translateY(4.5%) scale(.955) rotateX(-3deg);filter:brightness(.7) saturate(.75)!important;box-shadow:inset 0 0 0 2px #090502,inset 0 0 0 5px #3a2310,inset 0 13px 18px #000,inset 0 -3px 8px rgba(182,115,42,.12)}.action.is-depressed:before{box-shadow:inset 0 6px 9px #000,inset 0 -1px 2px rgba(255,205,108,.12)}.action.is-depressed .action-icon,.action.is-depressed strong{transform:translateY(2px)}.action.is-disabled{filter:brightness(.2) saturate(.25);opacity:.2}
    ...
    @media(max-width:340px){.action strong{font-size:7px}}
