...
<head>
  ...
  <link rel="preload" href="${MAHOGANY_FIXED_MARQUEE_ASSET}" as="image">
  <style>
    *{box-sizing:border-box}[hidden]{display:none!important}html{background:#030201}body{margin:0;min-height:100vh;background:#030201;color:#f4d99c;font-family:Georgia,"Times New Roman",serif}button,a{font:inherit}
    ...
    .marquee{position:absolute;z-index:3;top:0;left:0;width:100%;height:17.82%;overflow:hidden;pointer-events:none;filter:brightness(.58) saturate(.72);transition:filter 1.05s ease}.marquee img{display:block;width:100%;height:100%;object-fit:fill}.machine.is-powering .marquee,.machine.is-awake .marquee{filter:brightness(1.08) saturate(1.04) drop-shadow(0 0 3px rgba(239,156,67,.24))}
...
<body>
  <main>
    ...
    <section id="machine" class="machine" aria-label="${esc(input.name || "Edition")} Mahogany Jukebox preview">
      <header class="marquee" aria-label="${MAHOGANY_FIXED_MARQUEE}"><img src="${MAHOGANY_FIXED_MARQUEE_ASSET}" alt="" width="1280" height="413"></header>
