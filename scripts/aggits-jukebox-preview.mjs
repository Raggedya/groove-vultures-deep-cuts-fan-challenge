...
function fixedMarqueeMarkup() {
  return `<defs>
    <pattern id="aggitsBrassCells" width="18" height="16" patternUnits="userSpaceOnUse">
      <rect width="18" height="16" fill="#a96524"></rect>
      <path d="M4.5 0h9l4.5 8-4.5 8h-9L0 8z" fill="#bd7930" stroke="#e0a04d" stroke-width="1.15"></path>
      <path d="M4.5 1.8h9L16 8l-2.5 6.2h-9L2 8z" fill="none" stroke="#784015" stroke-width=".75" opacity=".78"></path>
    </pattern>
    <filter id="aggitsLetterDepth" x="-20%" y="-25%" width="140%" height="170%">
      <feDropShadow dx="0" dy="3" stdDeviation="1.4" flood-color="#160a02" flood-opacity="1"></feDropShadow>
      <feDropShadow dx="0" dy="-1" stdDeviation=".7" flood-color="#f1b963" flood-opacity=".92"></feDropShadow>
      <feDropShadow dx="0" dy="0" stdDeviation="3" flood-color="#c37a2d" flood-opacity=".28"></feDropShadow>
    </filter>
    <path id="aggitsMarqueeArc" d="M90 137 Q500 78 910 137"></path>
  </defs>
  <text id="fixedMarqueeTitle" aria-hidden="true" text-anchor="middle"><textPath href="#aggitsMarqueeArc" startOffset="50%">${MAHOGANY_FIXED_MARQUEE}</textPath></text>`;
}
...
export function renderAggitsJukeboxStudioPreview(
  ...
  const titleMarkup = fixedMarqueeMarkup();
  const ticker = (
...
<head>
  ...
  <style>
    *{box-sizing:border-box}[hidden]{display:none!important}html{background:#030201}body{margin:0;min-height:100vh;background:#030201;color:#f4d99c;font-family:Georgia,"Times New Roman",serif}button,a{font:inherit}.marquee svg{display:block;width:100%;height:86%;overflow:visible}.marquee #fixedMarqueeTitle{fill:url(#aggitsBrassCells);font:900 184px Georgia,"Times New Roman",serif;letter-spacing:.08em;paint-order:stroke;stroke:#3a1c08;stroke-width:4px;stroke-linejoin:round;opacity:.68;filter:url(#aggitsLetterDepth) brightness(.72) saturate(.78);transition:filter 1.05s ease,opacity 1.05s ease}.machine.is-powering .marquee #fixedMarqueeTitle,.machine.is-awake .marquee #fixedMarqueeTitle{opacity:1;filter:url(#aggitsLetterDepth) brightness(1.13) saturate(1.06) drop-shadow(0 0 3px rgba(244,171,76,.42))}
    ...
    .marquee{position:absolute;z-index:3;top:7.25%;left:12.7%;width:74.6%;height:10.3%;display:grid;align-content:center;justify-items:center;padding:1.2% 3.5% 0;overflow:hidden;border-radius:50% 50% 10% 10%/78% 78% 18% 18%;background:radial-gradient(ellipse at 50% 85%,#130c07 0,#070504 68%,#020201 100%);box-shadow:inset 0 -2px 5px #000}
...
<body>
  <main>
    ...
    <section id="machine" class="machine" aria-label="${esc(input.name || "Edition")} Mahogany Jukebox preview">
      <header class="marquee"><svg viewBox="0 0 1000 170" role="img" aria-label="${MAHOGANY_FIXED_MARQUEE}">${titleMarkup}</svg></header>
