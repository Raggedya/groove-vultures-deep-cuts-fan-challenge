...
export const MAHOGANY_RENDERER_VERSION =
  "mahogany-jukebox/2026-08-11-v9";
...
<head>
  ...
  <link rel="preload" href="${AGGITS_JUKEBOX_CANONICAL_BUTTON_ASSET}" as="image">
  <style>
    *{box-sizing:border-box}[hidden]{display:none!important}html{background:#030201}body{margin:0;min-height:100vh;background:#030201;color:#f4d99c;font-family:Georgia,"Times New Roman",serif}button,a{font:inherit}
    ...
    .actions{position:absolute;z-index:4;top:55.9%;left:12.05%;width:73.5%;height:19.02%;display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:0;padding:0;overflow:visible;isolation:isolate;background:#050201}.actions:before{position:absolute;z-index:0;inset:0;content:"";pointer-events:none;background:#050201 url("${AGGITS_JUKEBOX_CANONICAL_BUTTON_ASSET}") center/100% 100% no-repeat}.action{--bass-level:0;--bass-scale:1;position:relative;z-index:1;display:grid;place-items:center;min-width:0;min-height:0;padding:12%;overflow:visible;border:0;border-radius:50%;background:transparent;color:#d5a355;text-decoration:none;filter:brightness(.34) saturate(.5);opacity:1;pointer-events:none;transform:scale(var(--bass-scale));box-shadow:none;transition:filter .3s ease,transform .14s cubic-bezier(.2,.7,.2,1),box-shadow .14s ease}.action:after{position:absolute;z-index:0;inset:4.5% 5.5%;content:"";pointer-events:none;border-radius:50%;background:radial-gradient(ellipse at 50% 43%,rgba(255,213,132,.24),rgba(230,143,42,.09) 48%,transparent 72%);box-shadow:inset 0 0 clamp(7px,2vw,14px) rgba(255,186,73,.62),inset 0 0 0 clamp(1px,.35vw,2px) rgba(255,222,147,.68);opacity:calc(var(--bass-level) * .82);transition:opacity 70ms linear}.action-icon{position:relative;z-index:1;display:block;width:68%;aspect-ratio:1;overflow:visible}.action-icon img{position:absolute;top:50%;left:50%;display:block;width:100%;height:100%;max-width:none;object-fit:contain;transform:translate(-50%,-50%);filter:drop-shadow(0 2px 2px rgba(0,0,0,.8))}.machine.is-buttons-ready .action[data-enabled="true"]{filter:brightness(calc(.98 + var(--bass-level) * .18)) saturate(calc(.94 + var(--bass-level) * .12));opacity:1;pointer-events:auto}.action:focus-visible{outline:0;box-shadow:inset 0 0 0 clamp(2px,.55vw,4px) #ffe9ae,inset 0 0 0 clamp(4px,1vw,7px) #8b5725}.action.is-depressed{background:rgba(2,1,0,.44);transform:perspective(180px) translateY(4.5%) scale(.955) rotateX(-3deg);filter:brightness(.7) saturate(.75)!important;box-shadow:inset 0 12px 18px #000,inset 0 -2px 5px rgba(182,115,42,.11)}.action.is-depressed:after{opacity:0}.action.is-depressed .action-icon{transform:translateY(2px)}.action.is-disabled{filter:brightness(.18) saturate(.22);opacity:1}
