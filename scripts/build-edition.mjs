...
run(process.execPath,['scripts/validate-questions.mjs',slug]);
run(process.execPath,['scripts/test-engine.mjs',slug]);
run(process.execPath,['--check','js/app.js']);
run(process.execPath,['--check','js/engine.js']);
run(python,['scripts/ensure-python-deps.py']);
run(python,['scripts/generate-social-assets.py',slug]);
console.log(`${slug}: Deep Cuts build complete.`);
