...
for(const event of ['qr_scan','outbound_clicked','share_button_clicked','wheel_spin_started','wheel_result_shown','artist_destination_clicked','artist_directory_searched','artist_video_shown','quiz_started','quiz_question_answered','quiz_completed','quiz_abandoned','quiz_invitation_revealed','services_contact_clicked'])assert.ok(source.includes(`"${event}"`),`Worker event missing: ${event}`);
