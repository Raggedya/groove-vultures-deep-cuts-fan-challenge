...
http.createServer(async(request,response)=>{
  try{
    ...
    if(request.method==="POST"&&url.pathname==="/api/sell/identify"){
      const body=await jsonBody(request);return send(response,200,{ok:true,matches:telstraMatches(body.query,body.targetWebsite||body.website),source:"verified_demo"});
    }
    ...
    const relative=isAsset?url.pathname.replace(/^\/assets\//,""):url.pathname==="/sell/"||url.pathname==="/"?"index.html":url.pathname.replace(/^\/sell\//,"");
    const base=isAsset?assetRoot:root,file=path.resolve(base,relative);if(!file.startsWith(base))return send(response,403,{error:"Forbidden"});
