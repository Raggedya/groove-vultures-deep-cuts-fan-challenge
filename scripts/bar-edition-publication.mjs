...
function effectiveTicker(venue){return cleanMultiline(venue?.admin?.tickerOverride||venue?.automated?.generatedTicker?.tickerText||`${String(venue?.csv?.venueName||"").toUpperCase()} — ${[venue?.csv?.location,venue?.csv?.venueType].filter(Boolean).join(" — ").toUpperCase()}`,5000)}
