...
assert.match(barPreview,/<span class="share-word">SHARE<\/span>/);
...
assert.match(app,/saveVenueAndPublish/);assert.match(app,/\/save-publish/);assert.match(app,/waitForPublication/);
...
assert.match(venueHtml,/Add Venue/);
assert.match(venueHtml,/venue-published-toggle/);
...
assert.match(windowsBuilder,/assets\/aggits-jukebox-illuminated-master-v3\.png/);
assert.match(windowsBuilder,/jukebox-mechanical-button-clunk-public-domain\.ogg/);
assert.match(windowsBuilder,/iconFiles\.length!==173/);
...
assert.match(windowsBuilder,/"scripts\/venue-batch-publication\.mjs"/,"Windows packaging must include every desktop-main runtime import.");
assert.match(windowsBuilder,/--deep-cuts-bounded-smoke-test/,"Windows packaging must launch-test the packaged application with a bounded process.");
...
try{
  ...
  assert.equal(bootstrap.aggitsJukeboxIcons.length,173,"Studio must expose the complete approved Aggits Jukebox icon library.");
  ...
  assert.match(renderedBarPreview,/<span class="share-word">SHARE<\/span>/);
