...
assert.ok(contracts.productModels.bar_jukebox.lockedCapabilities.includes("three-stage-paired-incandescent-illumination"));
...
assert.match(html,/id="barJookBoxTitlePath" d="M 72 143 Q 360 55 648 143"/,"The venue title must use the protected inner arc and remain clear of the cabinet crown.");
...
assert.equal((html.match(/class="bar-jookbox-share-hand-ink"/g)||[]).length,2,"Both Share plaque hands require the approved horizontal silhouette.");
...
assert.match(app,/lightSequenceMode:"row-pair"/);
assert.match(app,/buttonLightDurationMs:Math\.max\(1200,Math\.min\(1800/);
assert.match(app,/function fitBarJookBoxMarqueeTitle\(\)/);
assert.match(app,/getComputedTextLength/);
assert.match(app,/const targetWidth=540/);
assert.match(app,/const maximumSize=76/);
...
assert.match(styles,/\.bar-jookbox-share-hand\{[\s\S]*?width:17%!important/);
...
assert.match(preview,/id="studioBarTitlePath" d="M72 143Q360 55 648 143"/);
...
assert.match(preview,/Math\.floor\(index\/2\)===keyIndex/);
assert.match(preview,/setTimeout\(sequence,1500\)/);
...
assert.match(publisher,/buttonLightDurationMs:1500/);
assert.match(publisher,/lightSequenceMode:"row-pair"/);
