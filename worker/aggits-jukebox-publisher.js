...
} from "../scripts/aggits-jukebox-button-master.mjs";
import {
...
export async function handleAggitsJukeboxPublicAsset(request, env, url) {
  ...
  if (qr && ["GET", "HEAD"].includes(request.method)) {
    const integrated = env.ASSETS
      ? await env.ASSETS.fetch(request).catch(() => null)
      : null;
    if (
      integrated?.ok &&
      (integrated.headers.get("content-type") || "")
        .toLowerCase()
        .includes("image/png")
    ) {
  ...
  const now = new Date().toISOString(),
    ...
    reserved = existing
      ...
      : await env.DB.prepare(
          "SELECT edition_id,slug FROM aggits_jukebox_publication_jobs WHERE project_id=?1 ORDER BY created_at ASC LIMIT 1",
        )
          .bind(manifest.value.projectId)
          .first(),
    editionId =
      existing?.edition_id || reserved?.edition_id || (await uniqueEditionId(env)),
    slug =
...
function buildConfig(job, m) {
  ...
  return {
    ...
    aggitsJukebox: {
      ...
      buttonArtwork: AGGITS_JUKEBOX_CANONICAL_BUTTON_ASSET.replace(/^\//, ""),
      buttonArtworkSha256: AGGITS_JUKEBOX_CANONICAL_BUTTON_SHA256,
      buttonArtworkVersion: AGGITS_JUKEBOX_CANONICAL_BUTTON_VERSION,
      ...
      value: {
        ...
        vu: {
          ...
          ducking: {
            ...
            holdMs: Math.max(400, Math.min(3000, Number(ducking.holdMs) || 1600)),
