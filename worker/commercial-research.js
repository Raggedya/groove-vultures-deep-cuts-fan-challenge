...
function nameFromPage(page,website){let value=String(page?.siteName||"").trim()||String(page?.title||"").split(/[|–—:]/)[0].trim();if(!value||value.length>80||/^(home|welcome|homepage|official site)$/i.test(value))value=new URL(website).hostname.replace(/^www\./,"").split(".")[0].replace(/[-_]+/g," ");return titleCase(value)}
