...
export async function buildCommercialReport({business,offering,request=""},env){
...
export async function buildUrlReview({url,reviewType="auto",spoilerFree=true,request=""},env){
...
export async function buildDocumentReview({text,fileName="document",reviewType="auto",request=""},env){
  if(!internalResearchReady(env))throw new ResearchError("RESEARCH_PROVIDER_UNAVAILABLE","Document review is not available at the moment.");
  const source=String(text||"").trim();
  if(source.length<40)throw new ResearchError("INVALID_DOCUMENT","The document does not contain enough readable text.");
  if(source.length>60000)throw new ResearchError("DOCUMENT_TOO_LARGE","The readable document text exceeds the 60,000-character analysis limit.");
  const kind=clean(reviewType,40)||"auto";
  const instruction=clean(request,1000)||"Explain the document clearly and identify the most useful next questions.";
  const result=await askJson(env,[
    {role:"system",content:"You are Banjo's careful document analyst. Use only the supplied document text. Never invent missing terms, obligations, dates, exclusions, prices, risks or legal conclusions. Distinguish document facts from interpretation and questions. This is practical information, not legal, financial or professional advice. Quote only short fragments when essential. Return JSON only."},
    {role:"user",content:`File: ${clean(fileName,240)}\nReview type: ${kind}\nUser request: ${instruction}\n\nCreate a clear 180-260 word spoken script in plain language. For a quote, identify scope, exclusions, assumptions, price or timing questions. For a tender, identify stated requirements, deadlines, dependencies, compliance points and unanswered questions. For a risk review, describe only risks supported by the text and label interpretations. For a summary, prioritise purpose, commitments, dates and next actions.\n\nReturn {"title":"...","documentType":"summary|risk|quote|tender|other","script":"...","findings":["..."],"questions":["..."],"limitations":["..."]}.\n\nDOCUMENT TEXT\n${source}`}
  ],1800);
  const script=clean(result?.script,6000);
  if(!script||script.split(/\s+/).length<40)throw new ResearchError("DOCUMENT_REVIEW_FAILED","The document was read, but a reliable review could not be assembled.");
  const list=value=>(Array.isArray(value)?value:[]).map(item=>clean(item,400)).filter(Boolean).slice(0,12);
  return {schemaVersion:"banjo-document-review/1.0",title:clean(result?.title,220)||clean(fileName,220),documentType:["summary","risk","quote","tender","other"].includes(result?.documentType)?result.documentType:"other",script,findings:list(result?.findings),questions:list(result?.questions),limitations:[...list(result?.limitations),"Review generated from extracted text only; confirm important terms in the original document."].slice(0,12),reviewedAt:new Date().toISOString(),researchMode:"owner_supplied_document_ai"};
}
