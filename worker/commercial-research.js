...
export async function identifyOfficialCompany(website,env){
  ...
  const root=pages[0];
  let profile={};
  if(internalResearchReady(env)){
    profile=await askJson(env,[
      {role:"system",content:"You identify a business only from its official website evidence. Return JSON only. Never invent missing details."},
      {role:"user",content:`Official URL: ${website}\nPage title: ${root.title}\nPage description: ${root.description}\nVisible text:\n${root.text.slice(0,7000)}\n\nReturn {"officialName":"...","tradingName":"...","industry":"...","location":"...","publicStatus":"..."}. Use "Not confirmed from the official website" for any genuinely unknown field.`}
    ],900).catch(()=>({}));
  }
  const officialName=clean(profile.officialName,160)||nameFromPage(root,website);
  return {
    id:`web-${slug(new URL(website).hostname)}`,
    officialName,
    tradingName:clean(profile.tradingName,160)||officialName,
    website,
    industry:clean(profile.industry,160)||"Industry not confirmed from the official website",
    location:clean(profile.location,160)||"Location not confirmed from the official website",
    publicStatus:clean(profile.publicStatus,80)||"Status not confirmed",
...
async function fetchPage(input,expectedHost){
  ...
  return {url:url.toString(),title:clean(decode(title),220),description:clean(decode(description),500),text,links:[...new Set(links)].slice(0,20)};
}
...
function normalizeInsight(item,key,sourceIds){
  ...
  return {title:clean(item?.title,180)||sectionFallbackTitle(key),status,found:clean(item?.found,800)||"The available official pages do not confirm this point.",meaning:clean(item?.meaning,600)||"There is not enough evidence for a firm interpretation.",relevance:clean(item?.relevance,600)||"Treat this as an open question, not a sales claim.",action:clean(item?.action,600)||"Validate this directly before shaping a proposal.",question:clean(item?.question,400)||"How does this work in practice today?",confidence:allowedConfidence.has(item?.confidence)?item.confidence:"Low confidence",sourceIds:ids};
}
...
function nameFromPage(page,website){let value=String(page?.title||"").split(/[|–—:]/)[0].trim();if(!value||value.length>80)value=new URL(website).hostname.replace(/^www\./,"").split(".")[0].replace(/[-_]+/g," ");return titleCase(value)}
