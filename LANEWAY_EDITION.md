...
The one-off is now a catalogue-discovery engine, not a Spotify directory. Its order is wheel, selected artist card, Surprise Me, curated related artists, searchable roster, ten-question catalogue quiz, licensing contact and footer. Every roster row selects the inline artist card; verified Spotify remains available as quick access. Each artist has explicit maintained recommendations in `artist-impact.json`, and quiz results use deterministic answer-based rules to lead back to the same discovery card. No recommendation is generated by AI at runtime.
