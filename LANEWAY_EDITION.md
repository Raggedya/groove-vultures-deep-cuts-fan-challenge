...
The separately typed `laneway_company` edition is an approved one-off and does not change the standard `Laneway [Band or Artist]` workflow above. It has a logo-only heading, an isolated 10-question deep-cut artist quiz, and a searchable artist directory. By explicit owner approval on 2026-07-26, this named one-off replaces its visible hero featured-video panel with an equal-chance artist-discovery wheel using the same verified Spotify roster. A selected artist may reveal a privacy-enhanced YouTube player beneath its impact and verified purchase controls only when the isolated artist-video map contains a currently playable, evidenced selection. Artists without one omit the panel. No other Laneway, Music, Cars, Clubs, Schools or Record Company product inherits the wheel.

By explicit owner approval on 2026-07-27, the production state restored by PR #102 is the final locked reference for `indie_label/1`. Future independent-label editions may reuse the interaction model only through isolated `indie_wheel` configuration. They must not modify this Laneway edition, restore the rejected catalogue-discovery overhaul or overwrite the completed Cool Death Records edition. See `INDIE_LABEL_MODEL.md`.
