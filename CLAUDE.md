...
Treat `JookBox [Band]` as the separately typed `jookbox/3` band model. Preserve every existing product. Never configure Aggits, a quiz or a wheel. Capture the artist-controlled Linktree as dated research evidence, independently verify every published destination and verify the most-viewed embeddable official YouTube video. Preserve the owner-locked full-height photoreal cabinet composition: top coin mechanism, user-controlled 16:9 screen, red Share hardware, amber left-to-right biography ticker, brass sourced-biography dial and at most eight explicit verified selection keys in four rows of two. The four states are `sleeping`, `acceptingCoin`, `poweringUp` and `awake`; coin audio is local and user-triggered, the awake flag is edition-specific session state, and YouTube never autoplays. Selection keys illuminate by row only when awake. Hidden tabs pause animation and reduced motion uses a gentle non-flashing treatment. Linktree is never scraped at runtime, and only IDs in the edition's curated `displaySelectionIds` may render.
