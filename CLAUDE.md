...
Treat Studio **Bar Edition** as the separately typed static `bar-jukebox/1` venue model. It reuses the locked ATLAS cabinet and shared coin/neon/CRT/ticker animation engine without changing Band Edition. Accept only administrator-supplied venue name, ticker, About Us copy, local MP4 and exactly five labelled HTTPS destinations; About Us is always the sixth key, and the support strip is the sole working `Share [Venue Name] with your mates` control. Never research, infer or scrape Bar Edition content. It has no Aggits, YouTube, quiz or wheel. Request local MP4 playback synchronously within the coin gesture and retain native controls for autoplay fallback. Never encode the private ephemeral `127.0.0.1` Studio preview in a Bar Edition QR; show publication pending and generate the permanent scan-tested QR only from the deployed public HTTPS URL.
