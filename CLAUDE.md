...
Treat `JookBox [Band]` as the separately typed `jookbox/2` band model. Preserve every existing product. Never configure Aggits, a quiz or a wheel. Capture the current artist-controlled Linktree as a dated build-time snapshot, independently verify every displayed destination and verify the most-viewed embeddable official YouTube video. Render the 16:9 video as the hero inside the coin-started, subtly weathered old-school cabinet; keep the dynamic band name more prominent than the small JookBox model mark, mount the coin and slot above the video, and mount Share beside the video metadata. Separate verified show destinations into one accessible Upcoming Shows module and render the remaining destinations as large self-balancing tactile controls. The user gesture must unlock links and initiate video playback. The dominant action promotes shows, with a verified music/video/site/newsletter fallback. Never render a fake transport control. `Learn More` must use sourced official biography copy. Reduced motion skips the theatrical start and makes ambient illumination static.
