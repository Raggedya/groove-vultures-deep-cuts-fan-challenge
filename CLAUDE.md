...
Treat Studio **Bar Edition** as the separately typed static `bar-jukebox/1` venue model. It reuses the locked ATLAS cabinet and shared coin/neon/CRT/ticker animation engine without changing Band Edition. Accept only administrator-supplied venue name, ticker, local MP4 and exactly five labelled HTTPS destinations; Share is always the sixth key. Never research, infer or scrape Bar Edition content. It has no Aggits, YouTube, quiz or wheel. Request local MP4 playback synchronously within the coin gesture and retain native controls for autoplay fallback.
