...
Treat `JookBox [Band]` as the separately typed `jookbox/3` band model. Preserve every existing product. Never configure Aggits, a quiz or a wheel. Capture the artist-controlled Linktree as dated research evidence, independently verify every published destination and verify the most-viewed embeddable official YouTube video. Preserve the owner-locked full-height photoreal cabinet composition: broad upper right-to-left amber biography ticker, privacy-enhanced 16:9 screen, centred post-video coin mechanism, red Share hardware, brass sourced-biography dial and at most eight explicit verified selection keys in four rows of two. The four states are `sleeping`, `acceptingCoin`, `poweringUp` and `awake`; coin audio is a sourced, licensed local real coin-slot recording and remains user-triggered, the awake flag is edition-specific session state, and YouTube requests autoplay synchronously within the direct coin interaction while retaining its manual play control for browsers that block it. Selection keys remain dimmed, unfocusable and unavailable before the coin, then illuminate brightly one at a time after activation and unlock only at `awake`. Hidden tabs pause animation and reduced motion uses a gentle non-flashing treatment. Linktree is never scraped at runtime, and only IDs in the edition's curated `displaySelectionIds` may render.
...
Preserve the isolated ATLAS exception at `dc_e22f1cb651`: `atlas-reference-cabinet/1` uses the locked 887 × 1774 ATLAS artwork and maps live content into its marquee, ticker, coin, video, three-by-two key bank, share panel and status hardware. Its large panel is the working `Support Our Band` / `Please share our JookBox` action, retaining the heart and adding a small share icon; the former lower Share hardware displays `Copyright Clearlight Creative 2026.` while the immutable global footer remains unchanged. Preserve its exact current key appearance byte-for-byte within the protected ATLAS CSS contract. Render its five verified destinations followed by Learn More under `six-key/1`, illuminate them gently one at a time in reading order, and never fabricate website or merchandise controls. This is not the JookBox factory default and must never affect the locked Filthy Animals reference.
