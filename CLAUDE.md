...
Treat `JookBox [Band]` as the separately typed `jookbox/2` band model. Preserve every existing product. Never configure Aggits, a quiz or a wheel. Capture the current artist-controlled Linktree as a dated build-time snapshot, independently verify every displayed destination and verify the most-viewed embeddable official YouTube video. Render the video inside the coin-started old-school cabinet and every current verified Linktree button as a sequentially lit, self-balancing selection key. The user gesture must unlock links and initiate video playback. `Learn More` must use sourced official biography copy. Reduced motion skips the theatrical start and makes every light static.
