...
Treat `JookBox [Band]` as the separately typed `jookbox/2` band model. Preserve every existing product. Never configure Aggits, a quiz or a wheel. Capture the current artist-controlled Linktree as dated research evidence, independently verify every published destination and verify the most-viewed embeddable official YouTube video. Render the 16:9 video as the hero inside the coin-started, subtly weathered old-school cabinet. The dynamic band name is a single-line, auto-fitting illuminated cabinet sign above the small JookBox mark; the coin and slot sit above the video, Share sits beside its one-line metadata, and duplicate plaque/helper/sound controls stay absent. Public destination controls are restricted to verified `Bandcamp`, `Spotify`, `Instagram`, `Band Website`, `Buy Music`, `Buy Merch` and `Facebook`, in that order; missing categories are omitted. Linktree must never introduce any other public button. The user gesture unlocks links and initiates video playback. `Learn More` is an inline cabinet selection beneath that deck, never a fixed bottom bar, and uses sourced official biography copy. Reduced motion skips the theatrical start and makes ambient illumination static.
