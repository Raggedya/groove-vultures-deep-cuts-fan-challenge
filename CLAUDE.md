...
Treat `JookBox [Band]` as the separately typed `jookbox/3` band model. Preserve every existing product. Never configure Aggits, a quiz or a wheel. Capture the artist-controlled Linktree as dated research evidence, independently verify every published destination and verify the most-viewed embeddable official YouTube video. Preserve the owner-locked full-height photoreal cabinet composition: top coin mechanism, privacy-enhanced 16:9 screen, red Share hardware, bright slightly enlarged right-to-left amber biography ticker, brass sourced-biography dial and at most eight explicit verified selection keys in four rows of two. The four states are `sleeping`, `acceptingCoin`, `poweringUp` and `awake`; coin audio is a sourced, licensed local real coin-slot recording and remains user-triggered, the awake flag is edition-specific session state, and YouTube requests autoplay synchronously within the direct coin interaction while retaining its manual play control for browsers that block it. Selection keys remain dimmed, unfocusable and unavailable before the coin, begin their staged illumination after activation and unlock only at `awake`. Hidden tabs pause animation and reduced motion uses a gentle non-flashing treatment. Linktree is never scraped at runtime, and only IDs in the edition's curated `displaySelectionIds` may render.
